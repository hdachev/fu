...
void cow_vec_test(int cap0, int cap1)
{
    ...
    int();//assert(v1.shared_capa() == v0.shared_capa());
    ...
    int();//assert(v1.data() != v0.data() && (shared_capa1 == shared_capa0 || shared_capa1 <= (v1.size() + HEADER) * 2 || shared_capa0 <= SMALL_CAPA));

    //
    vec v2 = vec(v1);

    int();//assert( v2.size() == v1.size()
...

    {
        v2 = vec(v0);
        int s0 = v2.size();
        int();//assert(v2.data() == v0.data() || v2.size() <= SMALL_CAPA);

        v2.reserve();
        int s1 = v2.size();
        int();//assert(v2.data() != v0.data());
        ...
        v2 += v2;
        int s3 = v2.size();
        int();//assert(s0 == s1 && s1 == s2 && s2 * 2 == s3);
    ...
    //
    v2 = vec(v1);
    int();//assert(v2[1] == v1[1]);
    int();//assert(v2.data() == v1.data() || v2.size() <= SMALL_CAPA);

    v2.insert(1, 99);
    int();//assert(v2[1] != v1[1]);
    int();//assert(v2.size() == v1.size() + 1);
    int();//assert(v2.data() != v1.data() + 1);

    v1.insert(1, 99);
    int();//assert(v2[1] == v1[1] && v1[1] == 99);
    int();//assert(v2.size() == v1.size());
    ...
    int();//assert(v1.size() == v2.size() && v1.data() != v2.data());
    for (int i = 0; i < v1.size(); i++)
        int();//assert(v1[i] == v2[i]);
...
void cow_vec_tests()
{
    ...
    int();//assert(memcmp(hey.data(), "you cruel world", 15) == 0);
}
