...
struct Argument_HFkq
{
    ...
    explicit operator bool() const noexcept
    {
        ...
        ;
                                ...
                                #ifndef DEF_ScopeSkip_ZjkMn5yAoeb
                                #define DEF_ScopeSkip_ZjkMn5yAoeb
struct ScopeSkip_ZjkM
{
    int start;
    int end;
...
Target_VZrr target_z0Qq(const ScopeItem_ydOE& si)
{
    return Target_VZrr { (si._packed & 0x3fffffffffffffffull) };
}

bool ScopeItem_shadows_z0Qq(const ScopeItem_ydOE& si)
{
    return !!(si._packed & 0x8000000000000000ull);
}

bool ScopeItem_canSkipRest_z0Qq(const ScopeItem_ydOE& si)
{
    return !!(si._packed & 0x4000000000000000ull);
}

uint64_t& ScopeItem_canSkipRest_set_z0Qq(ScopeItem_ydOE& si)
{
    return (si._packed |= 0x4000000000000000ull);
}

uint64_t& ScopeItem_canSkipRest_clear_z0Qq(ScopeItem_ydOE& si)
{
    return (si._packed &= ~0x4000000000000000ull);
}

ScopeItem_ydOE ScopeItem_z0Qq(fu::vec_range<char> id, const Target_VZrr& target, const bool shadows)
{
    return ScopeItem_ydOE { fu::str(id), ((target._packed & 0x3fffffffffffffffull) | (shadows ? 0x8000000000000000ull : 0ull)) };
}

                                #ifndef DEF___Yw7lA1TbL67
                                #define DEF___Yw7lA1TbL67
inline static const fu::vec<ScopeItem_ydOE>& _Yw7l(const Scope_bN0w& _)
{
    return _.items;
}
                                #endif

static uint64_t ScopeItem_canSkipRest_clear_FjkY(ScopeItem_ydOE& si)
{
    return (si._packed &= ~0x4000000000000000ull);
}

                                #ifndef DEF___cymgk7Jn9ui
                                #define DEF___cymgk7Jn9ui
inline static void _cymg(/*MOV*/ ScopeItem_ydOE&& item, fu::vec_range_mut<ScopeItem_ydOE> pUblic, fu::vec_range_mut<ScopeItem_ydOE> pRivate, const Scope_bN0w& scope, const int modid)
{
    Target_VZrr t;
    if (!((t = target_z0Qq(item), int(unsigned(((t._packed >> 40ull) & 0xfffffull)))) != modid))
    {
        ...
        Target_VZrr t_1;
        const Overload_OYmb& overload = scope.overloads[((t_1 = target_z0Qq(item), int(unsigned(((t_1._packed >> 20ull) & 0xfffffull)))) - 1)];
        ((overload.flags & Flags_Lzg8_F_PUB) ? pUblic : pRivate) += static_cast<ScopeItem_ydOE&&>(item);
    };
}
                                #endif

                                #ifndef DEF_each_iy7AMrpW6z8
                                #define DEF_each_iy7AMrpW6z8
inline void each_iy7A(fu::vec_range<ScopeItem_ydOE> a, fu::vec_range_mut<ScopeItem_ydOE> pUblic, fu::vec_range_mut<ScopeItem_ydOE> pRivate, const Scope_bN0w& scope, const int modid)
{
    for (int i = 0; i < a.size(); i++)
        _cymg(ScopeItem_ydOE(a[i]), pUblic, pRivate, scope, modid);

}
                                #endif

                                #ifndef DEF_collect_2RCVuMzCmia
                                #define DEF_collect_2RCVuMzCmia
inline static fu::vec<ScopeItem_ydOE> collect_2RCV(int& pub_cnt, /*MOV*/ fu::vec<ScopeItem_ydOE>&& pUblic, fu::vec<ScopeItem_ydOE>&& pRivate, const Scope_bN0w& scope, const int modid)
{
    each_iy7A(_Yw7l(scope), pUblic, pRivate, scope, modid);
    pub_cnt = pUblic.size();
    pUblic += pRivate;
    return pUblic.destructive_move();
                                ...
                                #ifndef DEF___ufrl9gBRU79
                                #define DEF___ufrl9gBRU79
inline static const fu::vec<ScopeItem_ydOE>& _ufrl(const Scope_bN0w& _)
{
    return _.implicits;
}
                                #endif

                                #ifndef DEF_collect_TWoVLAO5oF4
                                #define DEF_collect_TWoVLAO5oF4
inline static fu::vec<ScopeItem_ydOE> collect_TWoV(int& pub_cnt, /*MOV*/ fu::vec<ScopeItem_ydOE>&& pUblic, fu::vec<ScopeItem_ydOE>&& pRivate, const Scope_bN0w& scope, const int modid)
{
    each_iy7A(_ufrl(scope), pUblic, pRivate, scope, modid);
    pub_cnt = pUblic.size();
    pUblic += pRivate;
    return pUblic.destructive_move();
}
                                ...
                                #ifndef DEF___fn05XDpQ475
                                #define DEF___fn05XDpQ475
inline static const fu::vec<ScopeItem_ydOE>& _fn05(const Scope_bN0w& _)
{
    return _.globals;
}
                                #endif

                                #ifndef DEF_collect_7It8MR4Egta
                                #define DEF_collect_7It8MR4Egta
inline static fu::vec<ScopeItem_ydOE> collect_7It8(int& pub_cnt, /*MOV*/ fu::vec<ScopeItem_ydOE>&& pUblic, fu::vec<ScopeItem_ydOE>&& pRivate, const Scope_bN0w& scope, const int modid)
{
    each_iy7A(_fn05(scope), pUblic, pRivate, scope, modid);
    pub_cnt = pUblic.size();
    pUblic += pRivate;
    return pUblic.destructive_move();
}
...
Scope_bN0w Scope_exports_z0Qq(const Scope_bN0w& scope, const int modid, fu::vec_range<int> pub_imports)
{
    int pub_items = 0;
    int pub_implicits = 0;
    int pub_globals = 0;
    /*MOV*/ fu::vec<ScopeItem_ydOE> items = collect_2RCV(pub_items, fu::vec<ScopeItem_ydOE>{}, fu::vec<ScopeItem_ydOE>{}, scope, modid);
    /*MOV*/ fu::vec<ScopeItem_ydOE> implicits = collect_TWoV(pub_implicits, fu::vec<ScopeItem_ydOE>{}, fu::vec<ScopeItem_ydOE>{}, scope, modid);
    /*MOV*/ fu::vec<ScopeItem_ydOE> globals = collect_7It8(pub_globals, fu::vec<ScopeItem_ydOE>{}, fu::vec<ScopeItem_ydOE>{}, scope, modid);
    ...
    for (int i = 0; i < scope.converts.size(); i++)
    {
        ...
        {
            const Overload_OYmb& overload = scope.overloads[(int(unsigned(((item._packed >> 20ull) & 0xfffffull))) - 1)];
    ...
    return Scope_bN0w { fu::vec<Overload_OYmb>(scope.overloads), fu::vec<Extended_Ebxv>(scope.extended), items.destructive_move(), implicits.destructive_move(), globals.destructive_move(), fu::vec<int>(pub_imports), fu::vec<int>{}, fu::vec<Target_VZrr>{}, static_cast<fu::vec<Target_VZrr>&&>(converts), pub_items, pub_implicits, pub_globals, pub_converts };
}

static void nextSkip_z0Qq(fu::view<ScopeItem_ydOE> items, int& scope_iterator, fu::view<ScopeSkip_ZjkM> scope_skip, int& skiptrap)
{
    for (int i = scope_skip.size(); i-- > 0; )
    {
        const ScopeSkip_ZjkM& ss = scope_skip[i];
...
static uint64_t ScopeItem_canSkipRest_set_KHWe(ScopeItem_ydOE& si)
{
    return (si._packed |= 0x4000000000000000ull);
}

Target_VZrr search_z0Qq(fu::view_mut<ScopeItem_ydOE> items, fu::view<char> id, int& scope_iterator, fu::view<ScopeSkip_ZjkM> scope_skip, bool& shadows, fu::view<Target_VZrr> extra_items, fu::view<ScopeItem_ydOE> field_items, const TokenIdx_5b85& _here, const Context_UXD9& ctx)
{
    if ((scope_iterator >= 0))
    {
        shadows = false;
        ...
        if ((scope_iterator > int(0x10000000u)) && (scope_iterator < int(0x1fffffffu)))
        {
            ...
            while (scope_iterator-- > 0)
            {
                ...
                {
                    const ScopeItem_ydOE& item = items[scope_iterator];
                    if (item.id == id)
                    {
            ...
            {
                ScopeItem_ydOE& item = items.mutref(scope_iterator0);
                if (items[scope_iterator0].id == id)
                    ScopeItem_canSkipRest_set_KHWe(item);
                else
                    BUG_u9Gb((((("scope::search WILL_SKIP about to flag item.id("_view + item.id) + ") != search.id("_view) + id) + ")"_view), _here, ctx);
        ...
        if ((scope_iterator > int(0x60000000u)) && (scope_iterator < int(0x6fffffffu)))
        {
            ...
            while (scope_iterator-- > 0)
            {
                const ScopeItem_ydOE& item = field_items[scope_iterator];
                if (item.id == id)
                {
                    scope_iterator += int(0x60000000u);
                    return target_z0Qq(item);
        ...
        BUG_u9Gb("scope::search: Iterator already exhausted."_view, _here, ctx);
...
Target_VZrr search_KHWe(fu::view<ScopeItem_ydOE> items, fu::view<char> id, int& scope_iterator, fu::view<ScopeItem_ydOE> field_items)
{
    ...
    {
        const ScopeItem_ydOE& item = field_items[(scope_iterator++ - N0)];
        if (item.id == id)
            return target_z0Qq(item);
    ...
    return Target_VZrr{};
                                ...
                                #ifndef DEF_ensure_CMFV6qAiQx4
                                #define DEF_ensure_CMFV6qAiQx4
inline Extended_Ebxv& ensure_CMFV(fu::vec_range_mut<Extended_Ebxv> a, const int i)
{
    if ((a.size() <= i))
        a.grow((i + 1));
...
Target_VZrr Scope_create_z0Qq(Scope_bN0w& scope, const Kind_Idfg kind, fu::vec_range<char> name, const Type_KBIB& type, const Flags_Lzg8 flags, const SolverStatus_h9em status, const int nest, const bool asArgument, const Module_qqBh& module)
{
    int position {};
    Extended_Ebxv* ext;
    const Extended_Ebxv* ext_1;
    fu::vec<Overload_OYmb>& overloads = (nest ? (ext = &(ensure_CMFV(scope.extended, nest)), (position = (ext_1 = &((*ext)), (asArgument ? (-1 - (*ext_1).args_neg) : ((*ext_1).args_n_locals.size() + (1 - (*ext_1).args_neg))))), ((*ext).args_neg += (asArgument ? 1 : 0)), (*ext).args_n_locals) : ((position = (scope.overloads.size() + 1)), scope.overloads));
    const Target_VZrr target = Target_xQNS(module.modid, (nest ? nest : position), (!nest ? nest : position));
    /*MOV*/ Overload_OYmb item {};
    item.name = name;
    item.kind = kind;
    item.flags = flags;
    item.type = type;
    item.status = status;
    overloads.insert((asArgument ? 0 : overloads.size()), static_cast<Overload_OYmb&&>(item));
    if (!nest)
...
void Scope_set_FjkY(fu::vec_range_mut<ScopeItem_ydOE> items, fu::vec_range<char> id, const Target_VZrr& target, const bool shadows)
{
    items.push(ScopeItem_z0Qq(id, target, shadows));
}

void Scope_set_z0Qq(Scope_bN0w& scope, fu::vec_range<char> id, const Target_VZrr& target, const bool shadows)
{
