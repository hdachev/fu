...
struct s_ScopeSkip;
struct s_ScopeMemo;
fu::str createStructCanon_V0cIgHiE(s_kind, fu::view<char>, int, int);
unsigned speculateStruct_uflh0vv2(s_DeclAsserts, int);
...
int MODID_GjySIkmh(const s_Module& module)
{
    return module.modid;
}

s_Type initStruct_6PfOvswQ(const s_kind kind, const fu::str& baseprim, const fu::str& name, const s_DeclAsserts asserts, const int flatCount, fu::view<char> specPat, s_Module& module)
{
    if (fu::u8((fu::u8(fu::u8(name[0])) - fu::u8(fu::u8('0')))) > fu::u8(unsigned(9)))
    {
        ...
        /*MOV*/ fu::str canon = (createStructCanon_V0cIgHiE(kind, baseprim, module.modid, index) + specPat);
        return s_Type { s_ValueType { speculateStruct_uflh0vv2(asserts, flatCount), static_cast<fu::str&&>(canon) }, s_Lifetime{}, s_VFacts{} };
    }
    else
...
s_Scope Scope_exports_KDliHbWi(const s_Scope& scope, const int modid, const fu::vec<s_ScopeItem>& field_items, const fu::vec<int>& pub_imports)
{
    ...
    {
        /*MOV*/ s_ScopeItem item { scope.items[i] };
        int BL_3_v {};
        if ((__extension__ (
        {
            const s_Target t = target_NWTdzsfF(item);
            BL_3_v = (int(unsigned((t._packed & 0xfffffull))));
        (void)0;}), BL_3_v) == modid)
        {
            ...
            {
                const s_Target t = target_NWTdzsfF(item);
                BL_5_v = (int(unsigned(((t._packed >> 20ull) & 0xfffffull))));
            (void)0;}), BL_5_v) - 1)];
    ...
    for (int i_1 = 0; i_1 < scope.converts.size(); i_1++)
    {
        ...
        if (int(unsigned((item._packed & 0xfffffull))) == modid)
        {
...
static void nextSkip_clpwfFkQ(fu::view<s_ScopeItem> items, int& scope_iterator, fu::view<s_ScopeSkip> scope_skip, int& skiptrap)
{
...
s_Target search_ihIw5OEs(fu::view_mut<s_ScopeItem> items, fu::view<char> id, int& scope_iterator, fu::view<s_ScopeSkip> scope_skip, bool& shadows, fu::view<s_Target> extra_items, fu::view<s_ScopeItem> field_items)
{
...

    {
        ...
        scope_iterator--;
        nextSkip_clpwfFkQ(items, scope_iterator, scope_skip, skiptrap);
        scope_iterator++;
...
s_Target Scope_create_B5w8Fu3I(s_Scope& scope, const s_kind kind, const fu::str& name, const s_Type& type, const s_Flags flags, const s_SolverStatus status, const int nest, const s_Module& module)
{
    ...
    const s_Target target = Target_2jgCJZJY(MODID_GjySIkmh(module), (nest ? nest : position), (!nest ? nest : position));
    /*MOV*/ s_Overload item {};
    item.name = name;
    item.kind = kind;
    item.flags = flags;
