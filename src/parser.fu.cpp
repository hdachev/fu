...
inline Set_XshD uNion_liuZ(const Set_XshD&, const Set_XshD&);
...
static const Token_6M7a& consume_qFIn(const Kind_Idfg kind, fu::view<char> value, fu::vec_range<char> fname, fu::vec_range<char> src, fu::view<Token_6M7a> tokens, ParserState_Ahey& _ps)
{
    ...
    {
        fail_94Qv(((((("Expected "_view + qKW_e44U(str_PbEq(kind))) + ((value && (kind == token.kind)) ? (" "_view + qID_e44U(value)) : fu::str{})) + "\n\t     got "_view) + qKW_e44U(str_PbEq(token.kind))) + ((value && (kind == token.kind)) ? (" "_view + qID_e44U(token.value)) : fu::str{})), fname, src, tokens, _ps);
    };
}
...
static const Precedence_ybkz P_RESET fu_INIT_PRIORITY(1007) = Precedence_ybkz { 1000, 0, 0 };
                                ...
                                #ifndef DEF_tryConsume_zeneLqGIhNj
                                #define DEF_tryConsume_zeneLqGIhNj
inline static const Token_6M7a& tryConsume_zene(const Kind_Idfg kind, fu::view<char> value, const bool xqmark, fu::view<Token_6M7a> tokens, ParserState_Ahey& _ps)
{
    const Token_6M7a& token = tokens[_ps._idx];
...
inline Node_JjyR& last_krcn(fu::view_mut<Node_JjyR> s)
{
        ...
        else
        {
            if ((left.kind == Kind_Idfg_and) || (left.kind == Kind_Idfg_or))
            {
                if (!(left.syntax & ParseSyntax_Lay2_PS_PARENS))
                    warn_qFIn((((x7E_rA00("Unparenthesized `"_view, str_PbEq(left.kind)) + "` on the left side of an `"_view) + op_1) + "`."_view), fname, src, tokens, _ps, options);
...
inline const Node_JjyR& only_hN9E(fu::view<Node_JjyR> s)
{
    fu_ASSERT((s.size() == 1));
    return s[0];
}
...
inline static void right_X0YC(fu::vec_range<char> item, const int left, const Set_XshD& l, fu::vec<fu::str>& result, bool& useResult)
{
    ...
    result += fu::str(item);
                                ...
                                #ifndef DEF_merge_gQ4v4vPYm0f
                                #define DEF_merge_gQ4v4vPYm0f
inline static void merge_gQ4v(fu::view<fu::str> l, fu::view<fu::str> r, const Set_XshD& l_1, fu::vec<fu::str>& result, bool& useResult)
{
    int li = 0;
    int ri = 0;
    for (; ; )
    {
        ...
        else
        {
            ...
            ri++;
                                ...
                                #ifndef DEF_uNion_liuZyYvDyS2
                                #define DEF_uNion_liuZyYvDyS2
inline Set_XshD uNion_liuZ(const Set_XshD& l, const Set_XshD& r)
{
    if (r.keys_asc.size() > l.keys_asc.size())
        return uNion_liuZ(r, l);
    else
    {
        /*MOV*/ fu::vec<fu::str> result {};
        bool useResult = false;
        merge_gQ4v(l.keys_asc, r.keys_asc, l, result, useResult);
        if (useResult)
        {
...
inline const Node_JjyR& if_last_TSYP(fu::view<Node_JjyR> s)
{
    if (s.size())
        return s[(s.size() - 1)];
    else
...
inline const Node_JjyR& last_Flhi(fu::view<Node_JjyR> s)
{
    return s[(s.size() - 1)];
}
                                #endif
...
inline char only_ugv9(fu::view<char> s)
{
    fu_ASSERT((s.size() == 1));
    return s[0];
}
...
static Node_JjyR parseFor_qFIn(const int modid, fu::vec_range<char> fname, fu::vec_range<char> src, fu::vec_range<Token_6M7a> tokens, ParserState_Ahey& _ps, const Options_TBgD& options)
{
    ...
    else
    {
        ...
        if (init && (init.kind != Kind_Idfg_let))
            fail_94Qv((x7E_rA00("`for` expects a `let` statement, got a `"_view, str_PbEq(init.kind)) + "`."_view), fname, src, tokens, _ps);
        else
        {
...
static Node_JjyR parseJump_qFIn(const Kind_Idfg kind, const int modid, fu::vec_range<char> fname, fu::vec_range<char> src, fu::vec_range<Token_6M7a> tokens, ParserState_Ahey& _ps, const Options_TBgD& options)
{
...

    {
        ...
        if (!label)
        {
            if (kind == Kind_Idfg_break)
            {
                if (!(_ps.mbz._blockArgDepth || _ps.mbz._loopDepth))
                    fail_94Qv(((("Nothing to "_view + qKW_e44U(str_PbEq(kind))) + " from here."_view) + "\n\n\tNo surrounding loop or |code| block."_view), fname, src, tokens, _ps);
        ...
        {
            if (!(_ps.mbz._fnDepth))
                fail_94Qv(((("Cannot "_view + qKW_e44U(str_PbEq(kind))) + " here."_view) + "\n\n\tNo surrounding function."_view), fname, src, tokens, _ps);
...
static Node_JjyR parseUnwrap_qFIn(const int modid, fu::vec_range<char> fname, fu::vec_range<char> src, fu::vec_range<Token_6M7a> tokens, ParserState_Ahey& _ps, const Options_TBgD& options)
{
    ...
    if (stmt.kind == Kind_Idfg_block)
    {
        ...
        fail_94Qv((("Unwrap only works for blocks, this is a "_view + qKW_e44U(str_PbEq(stmt.kind))) + "."_view), fname, src, tokens, _ps);
...
{
    Set_XshD upstreamTypeParams0 = uNion_liuZ(_ps.mbz._upstreamTypeParams, _ps.mbz._expectedTypeParams);
...
static Node_JjyR parseLet_qFIn(const bool xqmark, const int anonIndex, const bool allowTypeUnions, const bool allowTypeParams, const Precedence_ybkz& initPrecedence, const int modid, fu::vec_range<char> fname, fu::vec_range<char> src, fu::vec_range<Token_6M7a> tokens, ParserState_Ahey& _ps, const Options_TBgD& options)
{
    ...
    {
        if (tryConsume_zene(Kind_Idfg_id, "using"_view, xqmark, tokens, _ps))
            flags |= Flags_Lzg8_F_USING;

        if (tryConsume_zene(Kind_Idfg_id, "implicit"_view, xqmark, tokens, _ps))
            flags |= Flags_Lzg8_F_IMPLICIT;

        if (tryConsume_zene(Kind_Idfg_id, "lax"_view, xqmark, tokens, _ps))
            flags |= Flags_Lzg8_F_LAX;

        if (tryConsume_zene(Kind_Idfg_id, "shadow"_view, xqmark, tokens, _ps))
            set_SHADOW_qFIn(flags, fname, src, tokens, _ps);

        if (tryConsume_zene(Kind_Idfg_id, "type"_view, xqmark, tokens, _ps))
            flags |= Flags_Lzg8_F_TYPENAME;
        else
        {
            if (tryConsume_zene(Kind_Idfg_id, "mut"_view, xqmark, tokens, _ps))
                flags |= Flags_Lzg8_F_MUT;
            else if (tryConsume_zene(Kind_Idfg_id, "const"_view, xqmark, tokens, _ps))
                flags |= Flags_Lzg8_F_CONST;

            if (tryConsume_zene(Kind_Idfg_id, "ref"_view, xqmark, tokens, _ps))
                flags |= Flags_Lzg8_F_REF;
            else if (tryConsume_zene(Kind_Idfg_id, "val"_view, xqmark, tokens, _ps))
