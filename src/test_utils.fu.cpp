...
inline const fu::str& last_StdcW3mS(fu::view<fu::str> s)
{
    return s[(s.size() - 1)];
                                ...
                                #ifndef DEF___G540127lcH8
                                #define DEF___G540127lcH8
inline static const fu::str& _G540127l(const s_Module& _)
{
    return _.out.cpp.src;
}
                                #endif

                                #ifndef DEF_map_5XdUvA4jTi8
                                #define DEF_map_5XdUvA4jTi8
inline fu::vec<fu::str> map_5XdUvA4j(fu::view<s_Module> a)
{
    /*MOV*/ fu::vec<fu::str> res {};
    res.grow<false>(a.size());
    for (int i = 0; i < a.size(); i++)
        res.mutref(i) = fu::str(_G540127l(a[i]));
...
s_Context compile_snippets_3QGLLDcU(fu::view<fu::str> sources, fu::vec<fu::str>&& fnames, s_Options&& options)
{
    ...
    /*MOV*/ s_Context ctx = clone_0ld3xnyz(CTX_PRELUDE);
    ...
    s_Context ctx_reverseBuildOrder = ((sources.size() > 1) ? clone_0ld3xnyz(ctx) : s_Context{});
    ...
    if (sources.size() > 1)
    {
        ...
        fu::vec<fu::str> fwd = map_5XdUvA4j(ctx.modules);
        fu::vec<fu::str> rev = map_5XdUvA4j(ctx_reverseBuildOrder.modules);
        sort_XEfD3Iab(fwd);
        sort_NIrE8Ph3(rev);
...
{
    return join_roEBocCS(map_5XdUvA4j(ctx.modules), "\n----\n"_fu);
}

                                #ifndef DEF_STOP_kon5kPVcRRi
                                #define DEF_STOP_kon5kPVcRRi
[[noreturn]] inline static fu::never STOP_kon5kPVc(fu::view<char> reason, fu::view<char> x, const s_Context& ctx, fu::view<fu::str> sources)
{
...
inline bool operator!=(fu::view<char> a, fu::view<char> b)
{
    return !!x3Cx3E_bv5nK4Kl(a, b);
                                ...
                                #ifndef DEF___aM2OArv7lYa
                                #define DEF___aM2OArv7lYa
inline static void _aM2OArv7(const fu::str& x, const s_Context& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if (!(found))
                    STOP_kon5kPVc("EXPECT mismatch"_fu, x, ctx, sources);

            }
            else if (cmd == "BROKEN"_fu)
            {
                if (found)
                    STOP_kon5kPVc("STILL BROKEN"_fu, x, ctx, sources);
                else
                    STOP_kon5kPVc("BROKEN mismatch"_fu, x, ctx, sources);

            }
            else if (cmd != "TODO"_fu)
                STOP_kon5kPVc("Unknown ;; CHECK"_fu, x, ctx, sources);
            else if (found)
                STOP_kon5kPVc("TODO test is actually passing"_fu, x, ctx, sources);
                                ...
                                #ifndef DEF_each_QqwjACNeTse
                                #define DEF_each_QqwjACNeTse
inline void each_QqwjACNe(fu::view<fu::str> a, const s_Context& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    for (int i = 0; i < a.size(); i++)
        _aM2OArv7(a[i], ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static s_TODO_FIX_Tuple compile_build_and_run_3QGLLDcU(fu::view<fu::str> sources, const s_Options& options, const int expect_exit, const s_DEV_OnFail onfail, const s_CGDefects ok_defects, fu::view<fu::str> expectations)
{
    ...
    each_QqwjACNe(expectations, ctx, expect_alloc_count, expect_alloc_bytes, sources);
