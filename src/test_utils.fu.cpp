...
inline const fu::str& last_StdcW3mS(fu::view<fu::str> s)
{
    return s[(s.size() - 1)];
                                ...
                                #ifndef DEF___0YgupJ5g8xc
                                #define DEF___0YgupJ5g8xc
inline static const fu::str& _0YgupJ5g(const s_Module& _)
{
    return _.out.cpp.src;
}
                                #endif

                                #ifndef DEF_map_fSGd01EaKP4
                                #define DEF_map_fSGd01EaKP4
inline fu::vec<fu::str> map_fSGd01Ea(fu::view<s_Module> a)
{
    /*MOV*/ fu::vec<fu::str> res {};
    res.grow<false>(a.size());
    for (int i = 0; i < a.size(); i++)
        res.mutref(i) = fu::str(_0YgupJ5g(a[i]));
...
s_Context compile_snippets_3QGLLDcU(fu::view<fu::str> sources, fu::vec<fu::str>&& fnames, s_Options&& options)
{
    ...
    /*MOV*/ s_Context ctx = clone_RvZs6BK0(CTX_PRELUDE);
    ...
    s_Context ctx_reverseBuildOrder = ((sources.size() > 1) ? clone_RvZs6BK0(ctx) : s_Context{});
    ...
    if (sources.size() > 1)
    {
        ...
        fu::vec<fu::str> fwd = map_fSGd01Ea(ctx.modules);
        fu::vec<fu::str> rev = map_fSGd01Ea(ctx_reverseBuildOrder.modules);
        sort_XEfD3Iab(fwd);
        sort_NIrE8Ph3(rev);
...
{
    return join_roEBocCS(map_fSGd01Ea(ctx.modules), "\n----\n"_fu);
}

                                #ifndef DEF_STOP_avt2JWTgUk4
                                #define DEF_STOP_avt2JWTgUk4
[[noreturn]] inline static fu::never STOP_avt2JWTg(fu::view<char> reason, fu::view<char> x, const s_Context& ctx, fu::view<fu::str> sources)
{
...
inline bool operator!=(fu::view<char> a, fu::view<char> b)
{
    return !!x3Cx3E_bv5nK4Kl(a, b);
                                ...
                                #ifndef DEF___4TKUdhNxxWc
                                #define DEF___4TKUdhNxxWc
inline static void _4TKUdhNx(const fu::str& x, const s_Context& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if (!(found))
                    STOP_avt2JWTg("EXPECT mismatch"_fu, x, ctx, sources);

            }
            else if (cmd == "BROKEN"_fu)
            {
                if (found)
                    STOP_avt2JWTg("STILL BROKEN"_fu, x, ctx, sources);
                else
                    STOP_avt2JWTg("BROKEN mismatch"_fu, x, ctx, sources);

            }
            else if (cmd != "TODO"_fu)
                STOP_avt2JWTg("Unknown ;; CHECK"_fu, x, ctx, sources);
            else if (found)
                STOP_avt2JWTg("TODO test is actually passing"_fu, x, ctx, sources);
                                ...
                                #ifndef DEF_each_zhUWvOP1hgf
                                #define DEF_each_zhUWvOP1hgf
inline void each_zhUWvOP1(fu::view<fu::str> a, const s_Context& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    for (int i = 0; i < a.size(); i++)
        _4TKUdhNx(a[i], ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static s_TODO_FIX_Tuple compile_build_and_run_3QGLLDcU(fu::view<fu::str> sources, const s_Options& options, const int expect_exit, const s_DEV_OnFail onfail, const s_CGDefects ok_defects, fu::view<fu::str> expectations)
{
    ...
    each_zhUWvOP1(expectations, ctx, expect_alloc_count, expect_alloc_bytes, sources);
    ...
    if (!s_DevOptions((options.dev & s_DevOptions_DEV_IgnoreDefects)))
    {
        ...
        const s_CGDefects unexpected = s_CGDefects((defects & s_CGDefects(~ok_defects)));
...
static void verify_errmsg_3QGLLDcU(fu::view<char> topic, const fu::str& e, const fu::str& msg)
{
    ...
    if (!BUG_ok && has_nst65ofM(e, "BUG"_fu))
...
s_Context ZERO_3QGLLDcU(fu::vec<fu::str>&& sources, s_Options&& options, const fu::str& expect_fail, const int expect_exit, s_TestDiffs& testdiffs)
{
    ...
    if (!has_nst65ofM(first_W2vsAJVm(sources), ";; EXPECT"_fu))
    {
