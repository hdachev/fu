...
struct TODO_FIX_Tuple_rHvr;
struct TestOutput_OIXu;
typedef int ExitStatus_iZrm;
typedef int Errno_88RA;
void compile_Tsi8(fu::vec_range<char>, Context_AtCH&, const Options_TBgD&);
Context_AtCH ZERO_3QGL(fu::vec<fu::str>&&, Options_TBgD&&, fu::vec_range<char>, int, TestDiffs_EVlo&);
...
struct TestOutput_OIXu
{
    ...
    explicit operator bool() const noexcept
    {
        ...
        ;
                                ...
                                #ifndef DEF_TODO_FIX_Tuple_rHvrixwgJqe
                                #define DEF_TODO_FIX_Tuple_rHvrixwgJqe
struct TODO_FIX_Tuple_rHvr
{
    Context_AtCH ctx;
    TestOutput_OIXu debug_output;
    TODO_FIX_Tuple_rHvr(const TODO_FIX_Tuple_rHvr&) = delete;
    TODO_FIX_Tuple_rHvr(TODO_FIX_Tuple_rHvr&&) = default;
    TODO_FIX_Tuple_rHvr& operator=(const TODO_FIX_Tuple_rHvr&) = delete;
    TODO_FIX_Tuple_rHvr& operator=(TODO_FIX_Tuple_rHvr&&) = default;
    explicit operator bool() const noexcept
    {
        return false
            || ctx
...
inline fu::vec_range<char> last_lPg0(fu::view<fu::str> s)
{
    return s[(s.size() - 1)];
                                ...
                                #ifndef DEF___LLzDeuaO1Lb
                                #define DEF___LLzDeuaO1Lb
inline static const fu::str& _LLzD(const Module_GQwr& _)
{
    return _.out.cpp.src;
}
                                #endif

                                #ifndef DEF_map_9KisoD1KEf6
                                #define DEF_map_9KisoD1KEf6
inline fu::vec<fu::str> map_9Kis(fu::view<Module_GQwr> a)
{
    /*MOV*/ fu::vec<fu::str> res {};
    res.grow<false>(a.size());
    for (int i = 0; i < a.size(); i++)
        res.mutref(i) = fu::str(_LLzD(a[i]));
...
Context_AtCH compile_snippets_3QGL(fu::view<fu::str> sources, fu::vec<fu::str>&& fnames, Options_TBgD&& options)
{
    options.dev |= DevOptions_QEya_DEV_CG_LifetimeAnnots;
    /*MOV*/ Context_AtCH ctx = clone_xQ6I(CTX_PRELUDE);
    ...
    for (int i_1 = 0; i_1 < sources.size(); i_1++)
    {
        ...
        set_Z3Rq(ctx.files, fnames[i_1], src);
        if (has_YIf6(snippet, "/*PRINT*/"_view))
            options.dev |= DevOptions_QEya_DEV_Print;

    };
    Context_AtCH ctx_reverseBuildOrder = ((sources.size() > 1) ? clone_xQ6I(ctx) : Context_AtCH{});
    ...
    if (sources.size() > 1)
    {
        ...
        fu::vec<fu::str> fwd = map_9Kis(ctx.modules);
        fu::vec<fu::str> rev = map_9Kis(ctx_reverseBuildOrder.modules);
        sort_h47d(fwd);
        sort_h47d(rev);
    ...
    {
        Module_GQwr& module = ctx.modules.mutref(i_2);
...
fu::str snippet2cpp_3QGL(fu::vec_range<char> src)
{
    ...
    Context_AtCH ctx = compile_snippets_3QGL(fu::view<fu::str> {{ fu::str(src) }}, fu::vec<fu::str> {{ fu::str(fname) }}, Options_TBgD{});
...
inline static fu::vec_range<char> this_src_yQFq(const int i, fu::view<fu::str> sources)
{
    return sources[i];
                                ...
                                #ifndef DEF_this_cpp_uAK4pb8QZc5
                                #define DEF_this_cpp_uAK4pb8QZc5
inline static fu::view<char> this_cpp_uAK4(const ModuleOutputs_mPlY& out)
{
    return out.cpp.src;
}
                                #endif

static fu::str all_cpp_3QGL(const Context_AtCH& ctx)
{
    return join_yqDb(map_9Kis(ctx.modules), "\n----\n"_view);
}

                                #ifndef DEF_STOP_SZSnu1LAVl8
                                #define DEF_STOP_SZSnu1LAVl8
[[noreturn]] inline static fu::never STOP_SZSn(fu::view<char> reason, fu::view<char> x, const Context_AtCH& ctx, fu::view<fu::str> sources)
{
...
inline bool operator!=(fu::view<char> a, fu::view<char> b)
{
    return !!x3Cx3E_j5CR(a, b);
                                ...
                                #ifndef DEF___HLGWwbUWQpf
                                #define DEF___HLGWwbUWQpf
inline static void _HLGW(fu::view<char> x, const Context_AtCH& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    ...
    else
    {
        ...
        else
        {
            ...
            for (int i = 0; i < sources.size(); i++)
            {
                const ModuleOutputs_mPlY& out = ctx.modules[(i + 1)].out;
                if (note)
                {
                ...
                else if ((found = has_YIf6(this_cpp_uAK4(out), rest)))
                    break;
            ...
            {
                if (!(found))
                    STOP_SZSn("EXPECT mismatch"_view, x, ctx, sources);

            }
            else if (cmd == "BROKEN"_view)
            {
                if (found)
                    STOP_SZSn("STILL BROKEN"_view, x, ctx, sources);
                else
                    STOP_SZSn("BROKEN mismatch"_view, x, ctx, sources);

            }
            else if (cmd != "TODO"_view)
                STOP_SZSn("Unknown ;; CHECK"_view, x, ctx, sources);
            else if (found)
                STOP_SZSn("TODO test is actually passing"_view, x, ctx, sources);
                                ...
                                #ifndef DEF_each_qUJYeCXfs8c
                                #define DEF_each_qUJYeCXfs8c
inline void each_qUJY(fu::view<fu::str> a, const Context_AtCH& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    for (int i = 0; i < a.size(); i++)
        _HLGW(a[i], ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static TODO_FIX_Tuple_rHvr compile_build_and_run_3QGL(fu::view<fu::str> sources, const Options_TBgD& options, const int expect_exit, const DEV_OnFail_3YLB onfail, const CGDefects_2L18 ok_defects, fu::view<fu::str> expectations)
{
    /*MOV*/ Context_AtCH ctx = compile_snippets_3QGL(sources, fu::vec<fu::str>{}, Options_TBgD(options));
    int expect_alloc_count = 0;
    int expect_alloc_bytes = 0;
    each_qUJY(expectations, ctx, expect_alloc_count, expect_alloc_bytes, sources);
    const bool run = true;
    fu::view<char> fudir = ctx.fudir;
    const TestOutput_OIXu debug_output = build_uGHE(fu::str(DEFAULT_WORKSPACE), fudir, fu::str{}, fu::str{}, fu::str{}, fu::str{}, fu::str{}, fu::view<char>{}, "debug"_view, onfail, run, expect_exit, false, false, false, fu::view<fu::str>{}, fu::view<fu::str>{}, ctx);
    build_uGHE(fu::str(DEFAULT_WORKSPACE), fudir, fu::str{}, fu::str{}, fu::str{}, fu::str{}, fu::str{}, fu::view<char>{}, fu::view<char>{}, onfail, run, expect_exit, false, false, false, fu::view<fu::str>{}, fu::view<fu::str>{}, ctx);
    ...
    if (!DevOptions_QEya((options.dev & DevOptions_QEya_DEV_IgnoreDefects)))
    {
        ...
        for (int i = 0; i < sources.size(); i++)
        {
            const CodegenOutput_kbk6& cpp = ctx.modules[(i + 1)].out.cpp;
            defects |= cpp.defects;
        };
    ...
    return TODO_FIX_Tuple_rHvr { static_cast<Context_AtCH&&>(ctx), TestOutput_OIXu(debug_output) };
}
...
static bool check_3QGL(fu::vec_range<char> word, fu::view<char> actual)
{
    ...
    {
        fu::str expect = lowercase_KPhi(fu::str(word));
...
{
    fu::str actual = lowercase_KPhi(ERR_STRIP_SNIPPETS_3QGL(ERR_STRIP_ANSI_3QGL(fu::str(ERR_TRIM_3QGL(e)))));
    bool BUG_ok = false;
    int m0 = -1;
...
Context_AtCH ZERO_3QGL(fu::vec<fu::str>&& sources, Options_TBgD&& options, fu::vec_range<char> expect_fail, const int expect_exit, TestDiffs_EVlo& testdiffs)
{
    for (int i = 0; i < sources.size(); i++)
    {
        for (; ; )
...
;
    ...
    {
        /*MOV*/ TODO_FIX_Tuple_rHvr _ = compile_build_and_run_3QGL(sources, options, expect_exit, onfail, ok_defects, expectations);

        {
            ...
            {
                input += sources[i_5];
                const Module_GQwr& m = _.ctx.modules[((i_5 + _.ctx.modules.size()) - sources.size())];
        ...
        return static_cast<Context_AtCH&&>(_.ctx);
    };
}
...
{
    fu::vec<Module_GQwr> expect = ZERO_3QGL(fu::vec<fu::str>(alts[0]), Options_TBgD{}, (*(fu::str*)fu::NIL), 0, testdiffs).modules;
    for (int i = 1; i < alts.size(); i++)
    {
        fu::vec<Module_GQwr> actual = compile_snippets_3QGL(alts[i], fu::vec<fu::str>{}, Options_TBgD{}).modules;
...
Context_AtCH ZERO_5TaN(fu::vec_range<char> src, TestDiffs_EVlo& testdiffs)
{
