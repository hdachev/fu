...
inline const fu::str& last_StdcW3mS(fu::view<fu::str> s)
{
    return s[(s.size() - 1)];
                                ...
                                #ifndef DEF___wZYvqJxWOS5
                                #define DEF___wZYvqJxWOS5
inline static const fu::str& _wZYvqJxW(const s_Module& _)
{
    return _.out.cpp.src;
}
                                #endif

                                #ifndef DEF_map_NNRrQIyeT54
                                #define DEF_map_NNRrQIyeT54
inline fu::vec<fu::str> map_NNRrQIye(fu::view<s_Module> a)
{
    /*MOV*/ fu::vec<fu::str> res {};
    res.grow<false>(a.size());
    for (int i = 0; i < a.size(); i++)
        res.mutref(i) = fu::str(_wZYvqJxW(a[i]));
...
inline void sort_NIrE8Ph3(fu::vec<fu::str>& a)
{
    sort_5TaNJXGQ(a);
                                ...
                                #ifndef DEF_str_fi0VzTiu2J4
                                #define DEF_str_fi0VzTiu2J4
inline fu::str str_fi0VzTiu(const s_SolverNotes n)
{
    /*MOV*/ fu::str res {};
...
s_Context compile_snippets_3QGLLDcU(fu::view<fu::str> sources, fu::vec<fu::str>&& fnames, s_Options&& options)
{
    ...
    /*MOV*/ s_Context ctx = clone_Vu5hNB9B(CTX_PRELUDE);
    ...
    s_Context ctx_reverseBuildOrder = ((sources.size() > 1) ? clone_Vu5hNB9B(ctx) : s_Context{});
    ...
    if (sources.size() > 1)
    {
        ...
        fu::vec<fu::str> fwd = map_NNRrQIye(ctx.modules);
        fu::vec<fu::str> rev = map_NNRrQIye(ctx_reverseBuildOrder.modules);
        sort_XEfD3Iab(fwd);
        sort_NIrE8Ph3(rev);
...
inline bool ends_JkFJYOhR(fu::view<char> a, fu::view<char> with)
{
    return (a.size() >= with.size()) && (fu::get_view(a, (a.size() - with.size()), a.size()) == with);
                                ...
                                #ifndef DEF_parse_7cVMbsoZJnd
                                #define DEF_parse_7cVMbsoZJnd
inline s_SolverNotes parse_7cVMbsoZ(fu::view<char> v)
{
    if (v == "N_FnResolve"_fu)
    ...
    return s_SolverNotes{};
                                ...
                                #ifndef DEF_parseWild_jOp2NG0YTTf
                                #define DEF_parseWild_jOp2NG0YTTf
inline s_SolverNotes parseWild_jOp2NG0Y(fu::view<char> v)
{
...
{
    return join_roEBocCS(map_NNRrQIye(ctx.modules), "\n----\n"_fu);
}

                                #ifndef DEF_STOP_TibY8IvhF48
                                #define DEF_STOP_TibY8IvhF48
[[noreturn]] inline static fu::never STOP_TibY8Ivh(fu::view<char> reason, fu::view<char> x, const s_Context& ctx, fu::view<fu::str> sources)
{
...
inline bool operator!=(fu::view<char> a, fu::view<char> b)
{
    return !!x3Cx3E_bv5nK4Kl(a, b);
                                ...
                                #ifndef DEF___WgMxnpeJidi
                                #define DEF___WgMxnpeJidi
inline static void _WgMxnpeJ(const fu::str& x, const s_Context& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    const s_SolverNotes note = parseWild_jOp2NG0Y(x);
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if (!(found))
                    STOP_TibY8Ivh("EXPECT mismatch"_fu, x, ctx, sources);

            }
            else if (cmd == "BROKEN"_fu)
            {
                if (found)
                    STOP_TibY8Ivh("STILL BROKEN"_fu, x, ctx, sources);
                else
                    STOP_TibY8Ivh("BROKEN mismatch"_fu, x, ctx, sources);

            }
            else if (cmd != "TODO"_fu)
                STOP_TibY8Ivh("Unknown ;; CHECK"_fu, x, ctx, sources);
            else if (found)
                STOP_TibY8Ivh("TODO test is actually passing"_fu, x, ctx, sources);
                                ...
                                #ifndef DEF_each_l1DZqCZnE8j
                                #define DEF_each_l1DZqCZnE8j
inline void each_l1DZqCZn(fu::view<fu::str> a, const s_Context& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    for (int i = 0; i < a.size(); i++)
        _WgMxnpeJ(a[i], ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static s_TODO_FIX_Tuple compile_build_and_run_3QGLLDcU(fu::view<fu::str> sources, const s_Options& options, const int expect_exit, const s_DEV_OnFail onfail, const s_CGDefects ok_defects, fu::view<fu::str> expectations)
{
    ...
    each_l1DZqCZn(expectations, ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static fu::str ERR_STRIP_SNIPPETS_3QGLLDcU(/*MOV*/ fu::str&& e, const bool append_snippet_counts)
{
    int i0 = 0;
    ...
    if (append_snippet_counts)
        e += x7E_3lDd4lqo((x7E_3lDd4lqo((x7E_3lDd4lqo("\nsnippets:"_fu, fu::i64dec((total_with_ctx + total_without_ctx))) + ":"_fu), fu::i64dec(total_with_ctx)) + ":"_fu), fu::i64dec(total_without_ctx));
...
{
    fu::str actual = lower_KPhiKsnb(ERR_STRIP_SNIPPETS_3QGLLDcU(ERR_STRIP_ANSI_3QGLLDcU(ERR_TRIM_3QGLLDcU(e)), has_nst65ofM(msg, "snippets:"_fu)));
    bool BUG_ok = false;
    int m0 = -1;
    ...
    for (int i_4 = 0; i_4 < sources.size(); i_4++)
    {
        ...
        while (((start = find_2C20a41o(src, " ;; "_fu, start)) >= 0))
        {
            ...
            if (annot[0] == '!')
            {
                ...
                const s_SolverNotes notes = parseWild_jOp2NG0Y(annot_1);
                if (notes)
