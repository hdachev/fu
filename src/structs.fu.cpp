...
StructCanon_nOQ0 parseStructCanon_p1TN(fu::view<char> canon)
{
    int offset = 1;
    ...
    {
        return StructCanon_nOQ0 { modid, (globid - ONE_BASED) };
    }
    else
...
bool isStruct_p1TN(const ValueType_B8uC& type)
{
    const char c = type.canon[0];
    return (fu::u8(fu::u8(c)) >= fu::u8(128u));
}

bool isStructOrUnion_p1TN(const ValueType_B8uC& type)
{
    return isStruct_p1TN(type);
}

int modidOfOrigin_p1TN(const Type_KBIB& type)
{
