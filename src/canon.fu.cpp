...
bool isCanonAssignable_Ot6wT82z(fu::view<char> host, fu::view<char> guest, const s_TokenIdx& _here, const s_Context& ctx)
{
    const bool guestHasPatterns = ends_AbHOxjqn(guest, ')');
    if (!guestHasPatterns)
    {
        if (starts_8EDTwZj1(guest, '['))
        {
            if (starts_8EDTwZj1(host, '['))
                return isCanonAssignable_Ot6wT82z(fu::get_view(host, 1, (host.size() - 1)), fu::get_view(guest, 1, (guest.size() - 1)), _here, ctx);
            else
                return false;
            ...
            BUG_u9Gbkniv((((("isCanonAssignable: host("_fu + host) + ") guest("_fu) + guest) + ")"_fu), _here, ctx);

    }
    else
    {
        const int g0 = (find_yP7FbxFZ(guest, '(') + 1);
        const int g1 = (guest.size() - 1);
        const bool hostHasPatterns = ends_AbHOxjqn(host, ')');
        if (!hostHasPatterns)
            return host == fu::get_view_start0(guest, (g0 - 1));
        else
        {
            const int h0 = (find_yP7FbxFZ(host, '(') + 1);
                    ...
                    for (; ; )
                    {
                        ...
                        { {
...

                            {
                                ...
                                for (; ; )
                                {
                                    const s_Target spec_of_1 = parseGlobal_xQNS19Zn(pattern_1, offset_1);
                                            ...
                                            else if (isDeep && isDeep_1)
                                            {
                                                ...
                                                do
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        if (isCanonAssignable_Ot6wT82z(hostItem, guestItem, _here, ctx))
                                                            goto BL_15;
                                                        else
                                                        {
                                                            h1_1++;
...
const fu::str& canon_tryIntersect_Ot6wT82z(const fu::str& a, const fu::str& b, const s_TokenIdx& _here, const s_Context& ctx)
{
    if (a.size() < b.size())
        return canon_tryIntersect_Ot6wT82z(b, a, _here, ctx);
    else if (isCanonAssignable_Ot6wT82z(a, b, _here, ctx))
        return a;
    else
...
const fu::str& canon_trySuper_Ot6wT82z(const fu::str& a, const fu::str& b, const s_TokenIdx& _here, const s_Context& ctx)
{
    if (a.size() > b.size())
        return canon_trySuper_Ot6wT82z(b, a, _here, ctx);
    else if (isCanonAssignable_Ot6wT82z(a, b, _here, ctx))
        return a;
    else
        return (*(const fu::str*)fu::NIL);
...
fu::view<char> tryGetPattern_Ot6wT82z(fu::view<char> canon)
{
    if (ends_AbHOxjqn(canon, ')'))
        return fu::get_view(canon, (find_yP7FbxFZ(canon, '(') + 1), (canon.size() - 1));
    else
    {
        return fu::view<char>{};
    };
...
void trimPattern_Ot6wT82z(fu::str& canon)
{
    const int x = find_yP7FbxFZ(canon, '(');
    if ((x >= 0))
        canon.shrink(x);
