...
{
    err_str_foi6(idx0, ((x7E_rA00(str_PbEq(kind), ": Unexpected "_view) + qBAD_e44U(fu::view<char> {{ src[reason] }})) + "."_view), src, fname, line, lidx, idx);
}
        ...
        else if ((c >= '0') && (c <= '9'))
        {
            ...
            else
            {
                ...
                if (hex && dot && !exp)
                    ...
                    token_pnA5(((dot || exp) ? Kind_Idfg_real : Kind_Idfg_int), lowercase_KPhi(fu::str(str)), idx0, idx1, line, lidx, tokens);
