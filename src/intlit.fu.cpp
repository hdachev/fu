...
Intlit_SPOq Intlit_MpuZ(const uint64_t absval, const bool negative, /*MOV*/ fu::str&& error, const bool uNsigned, const bool sIgned, const uint64_t base)
{
    if (!(error))
    ...
    return Intlit_SPOq { fu::u8(base), fu::u8(unsigned(minsize_i)), fu::u8(unsigned(minsize_u)), fu::u8(unsigned(minsize_f)), sIgned, uNsigned, negative, absval, error.destructive_move() };
}

Intlit_SPOq Intlit_u8re(fu::view<char> sign_prefix_value_suffix)
{
