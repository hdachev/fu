...
void _shrink_to_fit_jvxt(BitSet_mmp7& _)
{
...
void clear_O9VI(BitSet_mmp7& _, const int idx)
{
    ...
    _shrink_to_fit_jvxt(_);
}

                                #ifndef DEF_ensure_IKJqkhWG7x7
                                #define DEF_ensure_IKJqkhWG7x7
inline fu::u8& ensure_IKJq(fu::vec_range_mut<fu::u8> a, const int i)
{
    if ((a.size() <= i))
        a.grow((i + 1));
...
bool add_once_jvxt(BitSet_mmp7& _, const int idx)
{
    const int no_neg = (idx & MIN_FUlT());
    const int bucket = ((idx >> 3) | no_neg);
    const int bit = (idx & 7);
    const fu::u8 mask = fu::u8((fu::u8(1u) << fu::u8(unsigned(bit))));
    fu::u8& entry = ensure_IKJq(_._data, bucket);
    const bool ret = !fu::u8((entry & mask));
    entry |= mask;
    return ret;
}

void add_jvxt(BitSet_mmp7& _, const int idx)
{
    const int no_neg = (idx & MIN_FUlT());
    const int bucket = ((idx >> 3) | no_neg);
    const int bit = (idx & 7);
    const fu::u8 mask = fu::u8((fu::u8(1u) << fu::u8(unsigned(bit))));
    ensure_IKJq(_._data, bucket) |= mask;
}

void add_O9VI(BitSet_mmp7& _, const BitSet_mmp7& other)
{
...
[[noreturn]] fu::never rem_jvxt(BitSet_mmp7& _, const BitSet_mmp7& other)
{
    ...
    fu::fail(fu::str("NOT TESTED"_fu));
}

bool has_jvxt(const BitSet_mmp7& _, const int idx)
{
    const int no_neg = (idx & MIN_FUlT());
    const int bucket = ((idx >> 3) | no_neg);
    const int bit = (idx & 7);
...
bool rem_O9VI(BitSet_mmp7& _, const int idx)
{
    const int no_neg = (idx & MIN_FUlT());
    const int bucket = ((idx >> 3) | no_neg);
    const int bit = (idx & 7);
...
void add_range_jvxt(BitSet_mmp7& _, const int end)
{
...
int popcount_jvxt(const BitSet_mmp7& _)
{
    int sum = 0;
...
BitSet_mmp7& and_not_assign_jvxt(BitSet_mmp7& a, const BitSet_mmp7& b)
{
...
static const BitSet_mmp7& and_not_assign_SrAa(BitSet_mmp7& a, const BitSet_mmp7& b)
{
    const int N = min_DbqN(a._data.size(), b._data.size());
    for (int i = 0; i < N; i++)
        a._data.mutref(i) &= fu::u8(~b._data[i]);
...
BitSet_mmp7 negated_jvxt(const BitSet_mmp7& bitset, const int end)
{
    /*MOV*/ BitSet_mmp7 result {};
    add_range_jvxt(result, end);
    and_not_assign_SrAa(result, bitset);
    return /*NRVO*/ result;
}

bool has_inter_jvxt(const BitSet_mmp7& a, const BitSet_mmp7& b)
{
    const int N = min_DbqN(a._data.size(), b._data.size());
    for (int i = 0; i < N; i++)
    {
...
bool has_missing_jvxt(const BitSet_mmp7& a, const BitSet_mmp7& b)
{
