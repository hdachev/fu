...
s_Type VFactsAdd(/*MOV*/ s_Type&& t, const s_VFacts vfacts)
{
    ...
    return static_cast<s_Type&&>(t);
                                ...
                                #ifndef DEF_t_never_AssumingInfiniteRecursion
                                #define DEF_t_never_AssumingInfiniteRecursion
extern const s_Type t_never_AssumingInfiniteRecursion fu_INIT_PRIORITY(1009) = VFactsAdd(s_Type(t_never), s_VFacts_AssumingInfiniteRecursion);
                                #endif
...
s_Type createArray_9CJmuVSD(const s_Type& item)
{
    ...
    const unsigned quals = ((((item.vtype.quals & q_rx_copy) | q_rx_resize) | q_non_compund) | (is_void_9CJmuVSD(item.vtype) ? q_rx_copy : 0u));
...
extern const s_Type t_string_literal fu_INIT_PRIORITY(1009) = set_lifetime(s_Type(t_string), Lifetime_static_immoveable);
                                ...
                                #ifndef DEF_str_ELHFg9uz78h
                                #define DEF_str_ELHFg9uz78h
inline fu::str str_ELHFg9uz(const s_VFacts n)
{
    /*MOV*/ fu::str res {};

    {
        ...
        if (n & s_VFacts_AssumingInfiniteRecursion)
            res += ("AssumingInfiniteRecursion"_fu + ", "_fu);
...
bool propositionOK_9CJmuVSD(const s_Type& type, const bool vfactsOK, const s_TokenIdx& _here, const s_Context& ctx)
{
    if (!vfactsOK && type.vtype.vfacts)
        BUG_u9Gbkniv(x7E_3lDd4lqo("propositionOK seeing vfacts: "_fu, str_ELHFg9uz(type.vtype.vfacts)), _here, ctx);
...
s_VFacts isAssumingInfiniteRecursion_9CJmuVSD(const s_ValueType& t)
{
    return t.vfacts & s_VFacts_AssumingInfiniteRecursion;
}
...
bool areVFactsAssignable(const s_VFacts host, const s_VFacts guest, const s_TokenIdx& _here, const s_Context& ctx)
{

    {
        ...
        if (!(s_VFacts((h & g)) == h))
            BUG_u9Gbkniv(x7E_3lDd4lqo((x7E_3lDd4lqo("areVFactsAssignable: vfacts mismatch: "_fu, str_ELHFg9uz(host)) + " != "_fu), str_ELHFg9uz(guest)), _here, ctx);
...
fu::str explainVFacts_9CJmuVSD(const s_VFacts vfacts)
{
    ...
    if (vfacts & s_VFacts_AssumingInfiniteRecursion)
        result += (qVFACTS_e44UlzzA("assuming-infinite-recursion"_fu) + ' ');
