...
s_UserTypeCanon parseUserTypeCanon_27MV22ZA(fu::view<char> canon)
{
    int offset = 1;
    ...
    {
        return s_UserTypeCanon { modid, (globid - ONE_BASED) };
    }
    else
        fu::fail(x7E_3lDd4lqo((x7E_3lDd4lqo((("parseUserTypeCanon: trailing garbage in `"_fu + canon) + "`: "_fu), fu::i64dec(offset)) + "|"_fu), fu::i64dec(canon.size())));

}

fu::str createUserTypeCanon_27MV22ZA(const s_Kind kind, fu::view<char> basePrim, const int modid, const int index, fu::view<char> specPat, uint64_t hash)
{
    hash ^= (hash >> 32ull);
    hash ^= (hash >> 16ull);
    hash ^= (hash >> 8ull);
    ...
    if ((modid >= 0) && (index >= 0))
    {
        ...
        if (basePrim)
        {
            char& c = res.mutref(0);
            if ((c == 'e') || (c == 'm'))
                fu::fail(("createUserTypeCanon: bad basePrim: "_fu + basePrim));
        ...
        fu::fail("createUserTypeCanon: bad modid/index"_fu);

}

bool isUserType_27MV22ZA(const s_ValueType& type)
{
    const char c = type.canon[0];
    return (fu::u8(fu::u8(c)) >= fu::u8(128u));
}

int modidOfOrigin_27MV22ZA(const s_Type& type)
{
    if (isUserType_27MV22ZA(type.vtype))
    {
