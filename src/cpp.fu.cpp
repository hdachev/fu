...
fu_STR ID_YqscxHlw(const fu_STR& id)
{
    if (id.size() < KEYWORDS.size())
    {
        const int idx = find_IswWtOx1(KEYWORDS[id.size()], id);
        if ((idx >= 0))
        {
            /*MOV*/ fu_STR cpy { id };
...
bool hasAssignment_YqscxHlw(fu::view<char> op)
{
    return (op == "="_fu) || ((op.size() == 2) && fu::has(ASSIGN2, op)) || ((op.size() == 3) && fu::has(ASSIGN3, op));
}

bool hasBinary_YqscxHlw(fu::view<char> op)
{
    return fu::has(BINARY, op);
}

bool hasPostfix_YqscxHlw(fu::view<char> op)
{
    return (op == "postfix++"_fu) || (op == "postfix--"_fu);
}

bool hasUnary_YqscxHlw(fu::view<char> op)
{
    if (op.size() > 2)
        return hasPostfix_YqscxHlw(op);

    return fu::has(UNARY, op);
}

bool hasOperator_YqscxHlw(fu::view<char> op)
{
    return hasBinary_YqscxHlw(op) || hasUnary_YqscxHlw(op);
}
