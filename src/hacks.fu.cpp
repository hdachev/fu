...
fu::str serialize_UvH3gYAB(const s_ClosureID& _, const s_TokenIdx& _here, const s_Context& ctx)
{
    ...
    (void)0;}), BL_3_v);
    if ((globid <= 0))
        BUG_u9Gbkniv("ClosureID: bad globid"_fu, _here, ctx);
    else
    {
        appendVarint_V5IuMsej(result, unsigned(modid));
        appendVarint_V5IuMsej(result, _.revision);
        appendVarint_V5IuMsej(result, unsigned(globid));
        appendVarint_V5IuMsej(result, unsigned(locid));
        return /*NRVO*/ result;
    };
}

s_ClosureID tryParseClosureID_UvH3gYAB(fu::view<char> id, const s_TokenIdx& _here, const s_Context& ctx)
{
    if ((id.size() > 3) && (id[0] == '`'))
    {
        int offset = 1;
        fu_DEFER(if (!(offset == id.size()))
            BUG_u9Gbkniv("Bad ClosureID: trailing stuff."_fu, _here, ctx););
        const int modid = int(parseVarint_V5IuMsej(offset, id));
        const unsigned revision = parseVarint_V5IuMsej(offset, id);
        const int globid = int(parseVarint_V5IuMsej(offset, id));
        const int locid = int(parseVarint_V5IuMsej(offset, id));
        return s_ClosureID { Target_xQNS19Zn(modid, globid, locid), revision };
    }
    else
    {
        return s_ClosureID{};
...
s_NativeQualities NativeQualities_UvH3gYAB(fu::view<char> name)
{
    if (starts_hRi5tkxn(name, "\n#MayAlias"_fu))
    {
        return s_NativeQualities { 10, true };
    }
    else
    {
        return s_NativeQualities{};
    };
}
