...
fu::str resolveFile(const fu::str& from, fu::view<char> name, s_Context& ctx)
{
    ...
    else
    {
        ...
        return /*NRVO*/ resolve;
                                ...
                                #ifndef DEF_run_XWrg33Wke7e
                                #define DEF_run_XWrg33Wke7e
inline static fu::str run_XWrg33Wk(const fu::str& from, fu::view<char> name, fu::view<char> alt, fu::view<char> nested, s_Context& ctx)
{
    fu::str _0 {};
...
fu::str testSuite_resolvePrepopulatedFile(fu::view<char> from, fu::view<char> name, const s_Context& ctx)
{
    ...
    else
    {
        return fu::str{};
                                ...
                                #ifndef DEF_run_jHWVXWXJ0ce
                                #define DEF_run_jHWVXWXJ0ce
inline static fu::str run_jHWVXWXJ(fu::view<char> from, fu::view<char> name, fu::view<char> alt, fu::view<char> nested, const s_Context& ctx)
{
    fu::str _0 {};
...
fu::str resolveFile_SqDqFcFw(const fu::str& path, s_Context& ctx)
{
    ...
    if ((fuz_idx > 0) && (fuz_idx < (path.size() - 1)) && (find_tnDs1wBz(path, '\v', (fuz_idx + 1)) < 0))
    {
        ...
        fu::str _0 {};
        /*MOV*/ fu::str file = ((_0 = run_XWrg33Wk(from, name, alt, nested, ctx)) ? static_cast<fu::str&&>(_0) : run_jHWVXWXJ(from, name, alt, nested, ctx));
        if (file)
            return /*NRVO*/ file;
...
const s_UserType& lookupUserType_1qjplDUo(const s_UserTypeCanon& scp, const s_Module& module, const s_Context& ctx, const s_TokenIdx& _here)
{
...
const s_UserType& lookupUserType_SqDqFcFw(const s_ValueType& type, const s_TokenIdx& _here, const s_Context& ctx, const s_Module& module)
{
    return lookupUserType_1qjplDUo(parseUserTypeCanon_27MV22ZA(type.canon), module, ctx, _here);
}
...
bool is_recursive_1qjplDUo(const s_ValueType& type, const s_TokenIdx& _here, const s_Context& ctx, const s_Module& module)
{
    return lookupUserType_SqDqFcFw(type, _here, ctx, module).shape.recursive;
}

s_UserTypeCanon tryParseUserTypeCanon_1qjplDUo(const s_ValueType& type)
{
    if (isUserType_27MV22ZA(type) || (is_primitive_hxWWgdZ1(type) ? (basePrimPrefixLen_hxWWgdZ1(type.canon) < type.canon.size()) : false))
        return parseUserTypeCanon_27MV22ZA(type.canon);
    else
    {
        return s_UserTypeCanon{};
    };
}

const s_UserType& tryLookupUserType_1qjplDUo(const s_ValueType& type, const s_TokenIdx& _here, const s_Context& ctx, const s_Module& module)
{
    if (isUserType_27MV22ZA(type) || (is_primitive_hxWWgdZ1(type) ? (basePrimPrefixLen_hxWWgdZ1(type.canon) < type.canon.size()) : false))
        return lookupUserType_SqDqFcFw(type, _here, ctx, module);
    else
        return (*(const s_UserType*)fu::NIL);
...
{
    const s_UserType& s = tryLookupUserType_1qjplDUo(type, _here, ctx, module);
    if (s)
...
s_Shape getShape_1qjplDUo(const s_ValueType& type, const s_TokenIdx& _here, const s_Context& ctx, const s_Module& module)
{
    ...
    else if (is_primitive_hxWWgdZ1(type))
    {
        ...
        return s_Shape { fu::str(type.canon), 0ull, (__extension__ (
        {
            ...
            BL_3_v = (hash);
        (void)0;}), BL_3_v), 1, 0, s_Set_ukcriy4HHQd{}, false };
    }
    else if (is_sliceable_hxWWgdZ1(type))
    {
        ...
        return s_Shape { ""_fu, (item_shape.non_triv_mask | (1ull << (hash_1 & 63ull))), hash_1, 1, 0, s_Set_ukcriy4HHQd(item_shape.usertypes), item_shape.recursive };
    }
    else
    {
        return s_Shape{};
