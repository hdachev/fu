...
inline void add_D6pA(fu_VEC<int>& to, const int item)
{
...
struct sf_solve
{
    ...
    void FnDecl_init(const s_Target& target, const fu_STR& kind, const fu_STR& id, const s_Template& tEmplate)
    {
        s_Overload& overload = GET_mut(target);
        overload.name = id;
        overload.kind = kind;
        overload.tEmplate = tEmplate;
        Scope_set(_scope, id, target);
    };
    void FnDecl_update(const s_SolvedNode& solved)
    {
        ...
        s_Overload& overload = GET_mut(solved.target);
        const int args_len0 = overload.args.size();
        overload.type = ([&]() -> const s_Type& { { const s_Type& _ = items[(items.size() + FN_RET_BACK)].type; if (_) return _; } fail("FnDecl_update: no return type."_fu); }());
        overload.min = min;
        overload.max = max;
        overload.args = args;
        overload.flags = solved.flags;
        overload.solved = solved;
        if ((args_len0 != overload.args.size()))
        {
            for (int i = 0; (i < overload.used_by.size()); i++)
            {
                const s_Target userIdx = s_Target { int(module.modid), int(overload.used_by.mutref(i)) };
                s_Overload& user = GET_mut(userIdx);
                if (!(user.status & SS_DIRTY))
                {
                    ((user.status & SS_FINALIZED) && fail("Overload finalized, cannot make dirty."_fu));
    ...
    s_Target tryMatch__mutargs(const s_Scope& scope, fu_STR&& id, fu_VEC<s_SolvedNode>& args, const int flags, const s_Type& retType, const s_Target& target)
    {
        ...
        if (matchIdx)
        {
            ...
            for (int i = 0; (i < args.size()); i++)
            {
                ...
                {
                    const s_Argument& host_arg = host_args[i];
                    args.mutref(i) = ([&]() -> s_SolvedNode { { s_SolvedNode _ = s_SolvedNode(host_arg.dEfault); if (_) return _; } return bindImplicitArg(host_arg.name, host_arg.type, matchIdx); }());
                };
            };
        };
        return matchIdx;
    ...
    {
        return solved(node, t_void, solveNodes(node.items, t_void, s_Type{}, bool{}, true));
    };
    s_SolvedNode solveBlock(const s_Node& node, const s_Type& type)
    {
        ...
        fu_VEC<s_SolvedNode> items = solveNodes(node.items, t_void, type, expr, bool{});
    ...
    {
        fu_VEC<s_SolvedNode> items = solveNodes(node.items, s_Type{}, s_Type{}, bool{}, bool{});
    ...
    {
        return __solveFn(node, false, s_SolvedNode{}, bool{}, -1, s_Target{});
    };
    s_SolvedNode uSolveFn(const s_Node& node, const s_SolvedNode& prep)
    {
        return __solveFn(node, true, prep, bool{}, -1, s_Target{});
    };
    s_SolvedNode __solveFn(const s_Node& n_fn, const bool solve, const s_SolvedNode& prep, const bool spec, const int caseIdx, const s_Target& into)
    {
        if (spec)
        {
        }
        ...
        
        {
            ...
            if ((solve && !native))
            {
                ...
                (s_body || fail(fu_STR{}));
                outItems.mutref((outItems.size() + FN_BODY_BACK)) = (s_body ? s_body : fail(fu_STR{}));
            };
            std::swap(_current_fn, out);
            Scope_pop(_scope, scope0);
        ...
        if (!prep)
        {
            ...
            FnDecl_init(out.out.target, kind, name, tEmplate);
        };
        FnDecl_update(out.out);
        if (solve)
            return createFnDef(out.out);

        return std::move(out.out);
    };
    s_SolvedNode createFnDef(const s_SolvedNode& fn)
    {
    ...
    s_Target trySpecialize(const s_Target& target, const s_Template& tEmplate, const fu_VEC<s_SolvedNode>& args, const fu_STR& args_mangled)
    {
        ...
        if (!spec)
        {
            ...
            spec = ([&]() -> s_Target { { s_Target _ = doTrySpecialize(tEmplate, fu_VEC<s_SolvedNode>(args), mangle, s_Target{}); if (_) return _; } return s_Target(SPEC_FAIL); }());
    ...
    s_Target doTrySpecialize(const s_Template& tEmplate, fu_VEC<s_SolvedNode>&& args, fu_STR& mangle, const s_Target& into)
    {
        bool ok = true;
        ...
        s_SolvedNode prep = ([&]() -> s_SolvedNode { if (ok && !into) return __solveFn(tEmplate.node, false, s_SolvedNode{}, true, caseIdx, into); else return s_SolvedNode{}; }());
        if (prep.target)
            (module.out.specs.upsert(mangle) = prep.target);

        s_SolvedNode specialized = ([&]() -> s_SolvedNode { if (ok) return __solveFn(tEmplate.node, true, prep, true, caseIdx, into); else return s_SolvedNode{}; }());
        std::swap(_current_fn, current_fn0);
        Scope_pop(_scope, scope0);
    ...
    s_SolvedNode solveReturn(const s_Node& node)
    {
        ...
        s_SolvedNode out = solved(node, t_void, solveNodes(node.items, prevType, s_Type{}, bool{}, bool{}));
    ...
    {
        return s_Node { "call"_fu, 0, fu_STR(id), fu_VEC<s_Node>{}, s_TokenIdx((_here ? _here : fail(fu_STR{}))) };
    };
    ...
    s_SolvedNode solveCall(const s_Node& node, const s_Target& target)
    {
        ...
        fu_VEC<s_SolvedNode> args = solveNodes(node.items, s_Type{}, s_Type{}, bool{}, bool{});
    ...
    s_SolvedNode solveArrayLiteral(const s_Node& node, const s_Type& type)
    {
        ...
        fu_VEC<s_SolvedNode> items = solveNodes(node.items, itemType, s_Type{}, bool{}, bool{});
        int startAt = 0;
    ...
    s_Target injectImplicitArg(const fu_STR& id, const s_Type& type, const s_Target& becauseOf)
    {
        s_SolvedNode& fnNode = (_current_fn.out ? _current_fn.out : fail((((("No implicit `"_fu + id) + "` in scope, needed to call `"_fu) + GET(becauseOf).name) + "`."_fu)));
        const s_ScopeMemo scope0 = Scope_push(_scope);
    ...
    s_SolvedNode bindImplicitArg(const fu_STR& name, const s_Type& type, const s_Target& becauseOf)
    {
        const fu_STR& id = name;
        fu_VEC<s_SolvedNode> args {};
        const s_Target target = ([&]() -> s_Target { { s_Target _ = tryMatch__mutargs(_scope, fu_STR(id), args, 0, type, s_Target{}); if (_) return _; } { s_Target _ = injectImplicitArg(id, type, becauseOf); if (_) return _; } fail(fu_STR{}); }());
        ((GET(target).flags & F_IMPLICIT) || fail((((("Matching a non-implicit item in scope: `"_fu + name) + "`, binds to call to `"_fu) + GET(becauseOf).name) + "`."_fu)));
        return CallerNode(createRead("__implicit"_fu), s_Type(type), target, fu_VEC<s_SolvedNode>{});
    };
    ...
    {
        fu_VEC<s_SolvedNode> items = solveNodes(node.items, type, s_Type{}, bool{}, bool{});
        if (is_void(type))
            type = t_bool;
    ...
    s_SolvedNode CallerNode(const s_Node& node, s_Type&& type, const s_Target& target, fu_VEC<s_SolvedNode>&& args)
    {
        ...
        if (((overload.kind == "fn"_fu) && (target.modid == module.modid)))
        {
            const int index = (fu::has(_current_fn.closure_of, target.index) ? target.index : _current_fn.out.target.index);
            add_D6pA(GET_mut(target).used_by, index);
        };
    ...
    fu_VEC<s_SolvedNode> solveNodes(const fu_VEC<s_Node>& nodes, const s_Type& type_all, const s_Type& type_last, const bool use_type_last, const bool TEMP_open_templates)
    {
            ...
            
            {
                ...
                while (some)
                {
                    ...
                    for (int i_1 = i0; (i_1 < i1); i_1++)
                    {
                        ...
                        if (((r.kind != "fndef"_fu) || !resolveIfNeeded_nonTemplateFn(r.target)))
                        {
                            continue;
                        };
        ...
        ((result.size() == nodes.size()) || fail("solveNodes: result.len != nodes.len"_fu));
        _here = here0;
        return result;
    };
    bool resolveIfNeeded_nonTemplateFn(const s_Target& target)
    {
        s_Overload overload = GET(target);
        ...
        unorderedSolve(overload.tEmplate.node, overload.solved);
        _here = here0;
        return true;
    };
    bool resolveIfNeeded_templateSpec(const s_OpenTemplate& ot)
    {
        s_Overload overload = GET(ot.spec);
        if (!(overload.status & SS_DIRTY))
            return false;
        ...
        doTrySpecialize(ot.tEmplate, fu_VEC<s_SolvedNode>(ot.args), mangle, ot.spec);
        _here = here0;
        return true;
    };
    s_SolverOutput solve()
    {
        ...
        
        {
            ...
            (_current_fn && fail("non-empty _current_fn."_fu));
            ((SLOW_resolve == resolve_done) || fail("Some overload was left dirty."_fu));
