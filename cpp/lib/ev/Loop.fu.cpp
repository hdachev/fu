...
Errno_88RA sig_listen_m5Wn(const BlockedSigmask_g3Eo& sm, Loop_UQM7& event_loop)
{
    ...
    unsigned user32;
    const uint64_t user_data = (kind = Kind_2p91_blocked_signals, fd = sm.signal_fd, state = SocketState_xV3L{}, user32 = 0u, fu_ASSERT(((fd >= 0) ? (fd <= 0xffffff) : false)), (((uint64_t(kind) | (uint64_t(FD_q1SK(fd)) << 8ull)) | ((uint64_t(state) & STATE_MASK) << STATE_OFFSET)) | (uint64_t(user32) << 32ull)));
    Errno_88RA err {};
    #ifdef __linux__
        if (sm.signal_fd > 0) {
...
Errno_88RA wakeUp_forChild_m5Wn(const PID_05b1 pid, const FD_q1SK pid_fd, const WriteEnd_aNMe in_fd, const ReadEnd_6NTX out_fd, const ReadEnd_6NTX err_fd, const int event_id, const unsigned wants_streams, const bool initial, Loop_UQM7& event_loop)
{
    ...
    SocketState_xV3L state;
    unsigned user32;
    const uint64_t user_data_pid = (kind = Kind_2p91_child_process_pidfd, fd = FD_q1SK(event_id), state = SocketState_xV3L{}, user32 = 0u, fu_ASSERT(((fd >= 0) ? (fd <= 0xffffff) : false)), (((uint64_t(kind) | (uint64_t(FD_q1SK(fd)) << 8ull)) | ((uint64_t(state) & STATE_MASK) << STATE_OFFSET)) | (uint64_t(user32) << 32ull)));
