...
inline constexpr bool SELF_TEST = true;
                                #endif
const fu::str& _fname_xQNS(const TokenIdx_5581&, const Context_GwtN&);
fu::str relative_qZI0(fu::view<char>, fu::view<char>);
const Token_6M7a& _token_xQNS(const TokenIdx_5581&, const Context_GwtN&);
fu::vec_range<char> dirname_qZI0(fu::vec_range<char>);
fu::vec_range<char> ext_qZI0(fu::vec_range<char>);
fu::str formatCodeSnippet_k53p(fu::vec<TokenIdx_5581>&&, int, CodeFmt_snPx, const Context_GwtN&);
...
const fu::str& getModuleSrc_u9Gb(const int modid, const Context_GwtN& ctx)
{
...
fu::str formatTokenCoord_u9Gb(const TokenIdx_5581& token, const int from, const Context_GwtN& ctx)
{
...
inline const LineColChars_6JiM& first_o2sr(fu::view<LineColChars_6JiM> s)
{
    return s[0];
}
                                #endif
...
inline const LineColChars_6JiM& last_s4ZR(fu::view<LineColChars_6JiM> s)
{
    return s[(s.size() - 1)];
}
                                #endif
...
inline const TokenIdx_5581& if_first_dfES(fu::view<TokenIdx_5581> s)
{
    if (s.size())
        return s[0];
    else
...
fu::str formatCodeSnippet_k53p(fu::vec<TokenIdx_5581>&& tokens, const int from, const CodeFmt_snPx fmt, const Context_GwtN& ctx)
{
    sort_mysZ(tokens);
    /*MOV*/ fu::str append {};
...
fu::str FAIL_text_u9Gb(fu::vec_range<TokenIdx_5581> tokens, fu::view<char> reason, const Context_GwtN& ctx)
{
...
[[noreturn]] fu::never FAIL_u9Gb(fu::view<char> reason, fu::vec_range<TokenIdx_5581> tokens, const Context_GwtN& ctx)
{
...
[[noreturn]] fu::never BUG_u9Gb(fu::view<char> reason, const Context_GwtN& ctx, const TokenIdx_5581& _here)
{
