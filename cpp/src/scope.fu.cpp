...
Target_VZrr search_z0Qq(fu::view_mut<ScopeItem_xiLD> items, fu::view<char> id, int& scope_iterator, fu::view<ScopeSkip_iozY> scope_skip, bool& shadows, fu::view<Target_VZrr> extra_items, fu::view<ScopeItem_xiLD> field_items, const Context_Zsw9& ctx, const TokenIdx_5581& _here)
{
    if ((scope_iterator >= 0))
    {
        shadows = false;
...
Target_VZrr Scope_create_z0Qq(Scope_blWT& scope, const Kind_Idfg kind, fu::vec_range<char> name, const Type_OiTm& type, const Flags_Lzg8 flags, const DeclAsserts_taUG asserts, const SolverStatus_h9em status, const int nest, const bool asArgument, const Module_cdtC& module)
{
    int position {};
    Extended_z0HS* ext;
