...
void compile_Tsi8(fu::vec_range<char>, Context_GwtN&, const Options_TBgD&);
Context_GwtN ZERO_3QGL(fu::vec<fu::str>&&, Options_TBgD&&, fu::vec_range<char>, int, TestDiffs_EVlo&);
...
struct TestOutput_OIXu
{
    ...
    explicit operator bool() const noexcept
    {
        ...
        ;
                                ...
                                #ifndef DEF_TODO_FIX_Tuple_TwmCCuGIpJ5
                                #define DEF_TODO_FIX_Tuple_TwmCCuGIpJ5
struct TODO_FIX_Tuple_TwmC
{
    Context_GwtN ctx;
    TestOutput_OIXu debug_output;
    TODO_FIX_Tuple_TwmC(const TODO_FIX_Tuple_TwmC&) = delete;
    TODO_FIX_Tuple_TwmC(TODO_FIX_Tuple_TwmC&&) = default;
    TODO_FIX_Tuple_TwmC& operator=(const TODO_FIX_Tuple_TwmC&) = delete;
    TODO_FIX_Tuple_TwmC& operator=(TODO_FIX_Tuple_TwmC&&) = default;
    explicit operator bool() const noexcept
    {
        return false
            || ctx
...
inline fu::vec_range<char> last_lPg0(fu::view<fu::str> s)
{
    return s[(s.size() - 1)];
}
                                #endif

                                #ifndef DEF___auBfodZP553
                                #define DEF___auBfodZP553
inline static const fu::str& _auBf(const Module_nyjb& _)
{
    return _.out.cpp.src;
}
                                #endif

                                #ifndef DEF_map_CXETf0c49B3
                                #define DEF_map_CXETf0c49B3
inline fu::vec<fu::str> map_CXET(fu::view<Module_nyjb> a)
{
    /*MOV*/ fu::vec<fu::str> res {};
    res.grow<false>(a.size());
    for (int i = 0; i < a.size(); i++)
        res.mutref(i) = fu::str(_auBf(a[i]));
...
Context_GwtN compile_snippets_3QGL(fu::view<fu::str> sources, fu::vec<fu::str>&& fnames, Options_TBgD&& options)
{
    options.dev |= DevOptions_QEya_DEV_CG_LifetimeAnnots;
    /*MOV*/ Context_GwtN ctx = clone_PIlE(CTX_PRELUDE);
    ...
    Context_GwtN ctx_reverseBuildOrder = ((sources.size() > 1) ? clone_PIlE(ctx) : Context_GwtN{});
    ...
    if (sources.size() > 1)
    {
        ...
        fu::vec<fu::str> fwd = map_CXET(ctx.modules);
        fu::vec<fu::str> rev = map_CXET(ctx_reverseBuildOrder.modules);
        sort_h47d(fwd);
        sort_h47d(rev);
    ...
    {
        Module_nyjb& module = ctx.modules.mutref(i_2);
...
fu::str snippet2cpp_3QGL(fu::vec_range<char> src)
{
    ...
    Context_GwtN ctx = compile_snippets_3QGL(fu::view<fu::str> {{ fu::str(src) }}, fu::vec<fu::str> {{ fu::str(fname) }}, Options_TBgD{});
...
inline fu::vec_range<char> first_V7z2(fu::view<fu::str> s)
{
    return s[0];
}
                                #endif
...
inline static fu::vec_range<char> this_src_yQFq(const int i, fu::view<fu::str> sources)
{
    return sources[i];
                                ...
                                #ifndef DEF_this_cpp_HT51TBQsHy6
                                #define DEF_this_cpp_HT51TBQsHy6
inline static fu::view<char> this_cpp_HT51(const ModuleOutputs_65Tx& out)
{
    return out.cpp.src;
}
                                #endif

static fu::str all_cpp_3QGL(const Context_GwtN& ctx)
{
    return join_yqDb(map_CXET(ctx.modules), "\n----\n"_view);
}

                                #ifndef DEF_STOP_v93yq36nKQ2
                                #define DEF_STOP_v93yq36nKQ2
[[noreturn]] inline static fu::never STOP_v93y(fu::view<char> reason, fu::view<char> x, const Context_GwtN& ctx, fu::view<fu::str> sources)
{
...
inline bool operator!=(fu::view<char> a, fu::view<char> b)
{
    return !!x3Cx3E_j5CR(a, b);
                                ...
                                #ifndef DEF___I6KlHbvhdvi
                                #define DEF___I6KlHbvhdvi
inline static void _I6Kl(fu::view<char> x, const Context_GwtN& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    ...
    else
    {
        ...
        else
        {
            ...
            for (int i = 0; i < sources.size(); i++)
            {
                const ModuleOutputs_65Tx& out = ctx.modules[(i + 1)].out;
                if (note)
                {
                ...
                else if ((found = has_YIf6(this_cpp_HT51(out), rest)))
                    break;
            ...
            {
                if (!(found))
                    STOP_v93y("EXPECT mismatch"_view, x, ctx, sources);

            }
            else if (cmd == "BROKEN"_view)
            {
                if (found)
                    STOP_v93y("STILL BROKEN"_view, x, ctx, sources);
                else
                    STOP_v93y("BROKEN mismatch"_view, x, ctx, sources);

            }
            else if (cmd != "TODO"_view)
                STOP_v93y("Unknown ;; CHECK"_view, x, ctx, sources);
            else if (found)
                STOP_v93y("TODO test is actually passing"_view, x, ctx, sources);
                                ...
                                #ifndef DEF_each_dUDeAp0bwD0
                                #define DEF_each_dUDeAp0bwD0
inline void each_dUDe(fu::view<fu::str> a, const Context_GwtN& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    for (int i = 0; i < a.size(); i++)
        _I6Kl(a[i], ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static TODO_FIX_Tuple_TwmC compile_build_and_run_3QGL(fu::view<fu::str> sources, const Options_TBgD& options, const int expect_exit, const DEV_OnFail_3YLB onfail, const CGDefects_2L18 ok_defects, fu::view<fu::str> expectations)
{
    /*MOV*/ Context_GwtN ctx = compile_snippets_3QGL(sources, fu::vec<fu::str>{}, Options_TBgD(options));
    int expect_alloc_count = 0;
    int expect_alloc_bytes = 0;
    each_dUDe(expectations, ctx, expect_alloc_count, expect_alloc_bytes, sources);
    ...
    if (!DevOptions_QEya((options.dev & DevOptions_QEya_DEV_IgnoreDefects)))
    {
        ...
        for (int i = 0; i < sources.size(); i++)
        {
            const CodegenOutput_hNw4& cpp = ctx.modules[(i + 1)].out.cpp;
            defects |= cpp.defects;
        };
    ...
    return TODO_FIX_Tuple_TwmC { static_cast<Context_GwtN&&>(ctx), TestOutput_OIXu(debug_output.output) };
}
...
Context_GwtN ZERO_3QGL(fu::vec<fu::str>&& sources, Options_TBgD&& options, fu::vec_range<char> expect_fail, const int expect_exit, TestDiffs_EVlo& testdiffs)
{
    for (int i = 0; i < sources.size(); i++)
    {
        for (; ; )
    ...
    if (!has_YIf6(first_V7z2(sources), ";; EXPECT"_view))
    {
...
;
    ...
    {
        /*MOV*/ TODO_FIX_Tuple_TwmC _ = compile_build_and_run_3QGL(sources, options, expect_exit, onfail, ok_defects, expectations);

        {
            ...
            {
                input += sources[i_5];
                const Module_nyjb& m = _.ctx.modules[((i_5 + _.ctx.modules.size()) - sources.size())];
        ...
        return static_cast<Context_GwtN&&>(_.ctx);
    };
}
...
{
    fu::vec<Module_nyjb> expect = ZERO_3QGL(fu::vec<fu::str>(alts[0]), Options_TBgD{}, (*(fu::str*)fu::NIL), 0, testdiffs).modules;
    for (int i = 1; i < alts.size(); i++)
    {
        fu::vec<Module_nyjb> actual = compile_snippets_3QGL(alts[i], fu::vec<fu::str>{}, Options_TBgD{}).modules;
...
Context_GwtN ZERO_5TaN(fu::vec_range<char> src, TestDiffs_EVlo& testdiffs)
{
