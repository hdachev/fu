...
void compile_Tsi8(fu::vec_range<char>, Context_Dujv&, const Options_TBgD&);
...
struct TestOutput_OIXu
{
    ...
    explicit operator bool() const noexcept
    {
        ...
        ;
                                ...
                                #ifndef DEF_TODO_FIX_Tuple_kXY7Afn4fMj
                                #define DEF_TODO_FIX_Tuple_kXY7Afn4fMj
struct TODO_FIX_Tuple_kXY7
{
    Context_Dujv ctx;
    TestOutput_OIXu debug_output;
    TODO_FIX_Tuple_kXY7(const TODO_FIX_Tuple_kXY7&) = delete;
    TODO_FIX_Tuple_kXY7(TODO_FIX_Tuple_kXY7&&) = default;
    TODO_FIX_Tuple_kXY7& operator=(const TODO_FIX_Tuple_kXY7&) = delete;
    TODO_FIX_Tuple_kXY7& operator=(TODO_FIX_Tuple_kXY7&&) = default;
    explicit operator bool() const noexcept
    {
        return false
            || ctx
...
inline fu::vec_range<char> last_lPg0(fu::view<fu::str> a)
{
    return a[(a.size() - 1)];
                                ...
                                #ifndef DEF___BUIwNYnZFmf
                                #define DEF___BUIwNYnZFmf
inline static const fu::str& _BUIw(const Module_BOS4& _)
{
    return _.out.cpp.src;
}
                                #endif

                                #ifndef DEF_map_dmUUgQlA2s6
                                #define DEF_map_dmUUgQlA2s6
inline fu::vec<fu::str> map_dmUU(fu::view<Module_BOS4> a)
{
    /*MOV*/ fu::vec<fu::str> res {};
    res.grow<false>(a.size());
    for (int i = 0; i < a.size(); i++)
        res.mutref(i) = fu::str(_BUIw(a[i]));
...
Context_Dujv compile_snippets_3QGL(fu::view<fu::str> sources, fu::vec<fu::str>&& fnames, Options_TBgD&& options)
{
    options.dev |= DevOptions_QEya_DEV_CG_LifetimeAnnots;
    /*MOV*/ Context_Dujv ctx = clone_4EXu(CTX_PRELUDE);
    ...
    Context_Dujv ctx_reverseBuildOrder = ((sources.size() > 1) ? clone_4EXu(ctx) : Context_Dujv{});
    ...
    if (sources.size() > 1)
    {
        ...
        fu::vec<fu::str> fwd = map_dmUU(ctx.modules);
        fu::vec<fu::str> rev = map_dmUU(ctx_reverseBuildOrder.modules);
        sort_h47d(fwd);
        sort_h47d(rev);
...
fu::str snippet2cpp_3QGL(fu::vec_range<char> src)
{
    ...
    Context_Dujv ctx = compile_snippets_3QGL(fu::view<fu::str> {{ fu::str(src) }}, fu::vec<fu::str> {{ fu::str(fname) }}, Options_TBgD{});
...
inline bool ends_OW45(fu::view<char> a, fu::view<char> with)
{
    return (a.size() >= with.size()) && (fu::get_view(a, (a.size() - with.size()), a.size()) == with);
                                ...
                                #ifndef DEF_parse_HDpXXo8LlEl
                                #define DEF_parse_HDpXXo8LlEl
inline SolverNotes_LSla parse_HDpX(fu::view<char> v)
{
    if (v == "N_FnResolve"_view)
    ...
    return SolverNotes_LSla{};
                                ...
                                #ifndef DEF_parseWild_iaUQBd9IW07
                                #define DEF_parseWild_iaUQBd9IW07
inline SolverNotes_LSla parseWild_iaUQ(fu::view<char> v)
{
...
inline static fu::vec_range<char> this_src_yQFq(const int i, fu::view<fu::str> sources)
{
    return sources[i];
                                ...
                                #ifndef DEF_this_cpp_pbTOLzUhA3j
                                #define DEF_this_cpp_pbTOLzUhA3j
inline static fu::view<char> this_cpp_pbTO(const ModuleOutputs_0BsN& out)
{
    return out.cpp.src;
}
                                #endif

static fu::str all_cpp_3QGL(const Context_Dujv& ctx)
{
    return join_yqDb(map_dmUU(ctx.modules), "\n----\n"_view);
}

                                #ifndef DEF_STOP_PnfluFSq592
                                #define DEF_STOP_PnfluFSq592
[[noreturn]] inline static fu::never STOP_Pnfl(fu::view<char> reason, fu::view<char> x, const Context_Dujv& ctx, fu::view<fu::str> sources)
{
...
inline bool operator!=(fu::view<char> a, fu::view<char> b)
{
    return !!x3Cx3E_j5CR(a, b);
                                ...
                                #ifndef DEF___qZPJ4BHjtA4
                                #define DEF___qZPJ4BHjtA4
inline static void _qZPJ(fu::view<char> x, const Context_Dujv& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    const SolverNotes_LSla note = parseWild_iaUQ(x);
    ...
    else
    {
        ...
        else
        {
            ...
            for (int i = 0; i < sources.size(); i++)
            {
                const ModuleOutputs_0BsN& out = ctx.modules[(i + 1)].out;
                if (note)
                {
                ...
                else if ((found = has_YIf6(this_cpp_pbTO(out), rest)))
                    break;
            ...
            {
                if (!(found))
                    STOP_Pnfl("EXPECT mismatch"_view, x, ctx, sources);

            }
            else if (cmd == "BROKEN"_view)
            {
                if (found)
                    STOP_Pnfl("STILL BROKEN"_view, x, ctx, sources);
                else
                    STOP_Pnfl("BROKEN mismatch"_view, x, ctx, sources);

            }
            else if (cmd != "TODO"_view)
                STOP_Pnfl("Unknown ;; CHECK"_view, x, ctx, sources);
            else if (found)
                STOP_Pnfl("TODO test is actually passing"_view, x, ctx, sources);
                                ...
                                #ifndef DEF_each_5dR8pzDnN1f
                                #define DEF_each_5dR8pzDnN1f
inline void each_5dR8(fu::view<fu::str> a, const Context_Dujv& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    for (int i = 0; i < a.size(); i++)
        _qZPJ(a[i], ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static TODO_FIX_Tuple_kXY7 compile_build_and_run_3QGL(fu::view<fu::str> sources, const Options_TBgD& options, const int expect_exit, const DEV_OnFail_3YLB onfail, const CGDefects_2L18 ok_defects, fu::view<fu::str> expectations)
{
    /*MOV*/ Context_Dujv ctx = compile_snippets_3QGL(sources, fu::vec<fu::str>{}, Options_TBgD(options));
    int expect_alloc_count = 0;
    int expect_alloc_bytes = 0;
    each_5dR8(expectations, ctx, expect_alloc_count, expect_alloc_bytes, sources);
    ...
    return TODO_FIX_Tuple_kXY7 { static_cast<Context_Dujv&&>(ctx), TestOutput_OIXu(debug_output.output) };
}
...
static fu::str ERR_STRIP_SNIPPETS_3QGL(/*MOV*/ fu::str&& e, const bool append_snippet_counts)
{
    int i0 = 0;
    int total_with_ctx = 0;
    int total_without_ctx = 0;
    int curr_snippet_lines = 0;
    for (int i = 0; i < e.size(); i++)
    {
        const char c = e[i];
        if (!((c == ' ') || (c == '\n')))
        {
            ...
            for (; i < e.size(); i++)
            {
                ...
                if (c_1 == '\n')
                {
                    ...
                    {
                        i0 = i;
                        if (curr_snippet_lines)
                        {
                            if (curr_snippet_lines > 1)
                                total_with_ctx++;
                            else
                                total_without_ctx++;

                            curr_snippet_lines = 0;
                        };
                    };
                    i--;
                    break;
    ...
    if (append_snippet_counts)
        e += x7E_rA00((x7E_rA00((x7E_rA00("\nsnippets:"_view, fu::i64dec((total_with_ctx + total_without_ctx))) + ":"_view), fu::i64dec(total_with_ctx)) + ":"_view), fu::i64dec(total_without_ctx));
...
{
    fu::str actual = lower_KPhi(ERR_STRIP_SNIPPETS_3QGL(ERR_STRIP_ANSI_3QGL(fu::str(ERR_TRIM_3QGL(e))), has_YIf6(msg, "snippets:"_view)));
    bool BUG_ok = false;
    int m0 = -1;
...
Context_Dujv ZERO_3QGL(fu::vec<fu::str>&& sources, Options_TBgD&& options, fu::vec_range<char> expect_fail, const int expect_exit, TestDiffs_EVlo& testdiffs)
{
    for (int i = 0; i < sources.size(); i++)
    {
        for (; ; )
    ...
    for (int i_4 = 0; i_4 < sources.size(); i_4++)
    {
        ...
        while (((start = find_BNNw(src, " ;; "_view, start)) >= 0))
        {
            ...
            if (annot[0] == '!')
            {
                ...
                const SolverNotes_LSla notes = parseWild_iaUQ(annot_1);
                if (notes)
...
;
    ...
    {
        /*MOV*/ TODO_FIX_Tuple_kXY7 _ = compile_build_and_run_3QGL(sources, options, expect_exit, onfail, ok_defects, expectations);

        {
            ...
            {
                input += sources[i_5];
                const Module_BOS4& m = _.ctx.modules[((i_5 + _.ctx.modules.size()) - sources.size())];
        ...
        return static_cast<Context_Dujv&&>(_.ctx);
    };
}
...
{
    fu::vec<Module_BOS4> expect = ZERO_3QGL(fu::vec<fu::str>(alts[0]), Options_TBgD{}, (*(fu::str*)fu::NIL), 0, testdiffs).modules;
    for (int i = 1; i < alts.size(); i++)
    {
        fu::vec<Module_BOS4> actual = compile_snippets_3QGL(alts[i], fu::vec<fu::str>{}, Options_TBgD{}).modules;
...
Context_Dujv ZERO_5TaN(fu::vec_range<char> src, TestDiffs_EVlo& testdiffs)
{
