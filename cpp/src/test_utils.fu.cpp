...
inline bool operator!=(fu::view<char> a, fu::view<char> b)
{
    return !!x3Cx3E_j5CR(a, b);
                                ...
                                #ifndef DEF___iZZT19wKNvj
                                #define DEF___iZZT19wKNvj
inline static void _iZZT(fu::view<char> x, const Context_noPA& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
        ...
        else
        {
            ...
            else if (cmd == "BROKEN"_view)
            {
                ...
                STOP_OP73("TODO test is actually passing"_view, x, ctx, sources);
                                ...
                                #ifndef DEF_each_WAfuusD8enl
                                #define DEF_each_WAfuusD8enl
inline void each_WAfu(fu::view<fu::str> a, const Context_noPA& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    for (int i = 0; i < a.size(); i++)
        _iZZT(a[i], ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static TODO_FIX_Tuple_e6jI compile_build_and_run_3QGL(fu::view<fu::str> sources, const Options_TBgD& options, const int expect_exit, const DEV_OnFail_3YLB onfail, const CGDefects_2L18 ok_defects, fu::view<fu::str> expectations)
{
    ...
    each_WAfu(expectations, ctx, expect_alloc_count, expect_alloc_bytes, sources);
