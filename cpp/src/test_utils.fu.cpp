...
void compile_Tsi8(fu::vec_range<char>, Context_Zsw9&, const Options_TBgD&);
Context_Zsw9 ZERO_3QGL(fu::vec<fu::str>&&, Options_TBgD&&, fu::vec_range<char>, int, TestDiffs_EVlo&);
...
struct TestOutput_OIXu
{
    ...
    explicit operator bool() const noexcept
    {
        ...
        ;
                                ...
                                #ifndef DEF_TODO_FIX_Tuple_fZNPBKTo1Ch
                                #define DEF_TODO_FIX_Tuple_fZNPBKTo1Ch
struct TODO_FIX_Tuple_fZNP
{
    Context_Zsw9 ctx;
    TestOutput_OIXu debug_output;
    TODO_FIX_Tuple_fZNP(const TODO_FIX_Tuple_fZNP&) = delete;
    TODO_FIX_Tuple_fZNP(TODO_FIX_Tuple_fZNP&&) = default;
    TODO_FIX_Tuple_fZNP& operator=(const TODO_FIX_Tuple_fZNP&) = delete;
    TODO_FIX_Tuple_fZNP& operator=(TODO_FIX_Tuple_fZNP&&) = default;
    explicit operator bool() const noexcept
    {
        return false
            || ctx
...
inline fu::vec_range<char> last_lPg0(fu::view<fu::str> a)
{
    return a[(a.size() - 1)];
                                ...
                                #ifndef DEF___PBC17CsxkPg
                                #define DEF___PBC17CsxkPg
inline static const fu::str& _PBC1(const Module_cdtC& _)
{
    return _.out.cpp.src;
}
                                #endif

                                #ifndef DEF_map_4Hg5FodMXmi
                                #define DEF_map_4Hg5FodMXmi
inline fu::vec<fu::str> map_4Hg5(fu::view<Module_cdtC> a)
{
    /*MOV*/ fu::vec<fu::str> res {};
    res.grow<false>(a.size());
    for (int i = 0; i < a.size(); i++)
        res.mutref(i) = fu::str(_PBC1(a[i]));
...
Context_Zsw9 compile_snippets_3QGL(fu::view<fu::str> sources, fu::vec<fu::str>&& fnames, Options_TBgD&& options)
{
    options.dev |= DevOptions_QEya_DEV_CG_LifetimeAnnots;
    /*MOV*/ Context_Zsw9 ctx = clone_4uiE(CTX_PRELUDE);
    ...
    Context_Zsw9 ctx_reverseBuildOrder = ((sources.size() > 1) ? clone_4uiE(ctx) : Context_Zsw9{});
    ...
    if (sources.size() > 1)
    {
        ...
        fu::vec<fu::str> fwd = map_4Hg5(ctx.modules);
        fu::vec<fu::str> rev = map_4Hg5(ctx_reverseBuildOrder.modules);
        sort_h47d(fwd);
        sort_h47d(rev);
    ...
    {
        Module_cdtC& module = ctx.modules.mutref(i_2);
...
fu::str snippet2cpp_3QGL(fu::vec_range<char> src)
{
    ...
    Context_Zsw9 ctx = compile_snippets_3QGL(fu::view<fu::str> {{ fu::str(src) }}, fu::vec<fu::str> {{ fu::str(fname) }}, Options_TBgD{});
...
inline static fu::vec_range<char> this_src_yQFq(const int i, fu::view<fu::str> sources)
{
    return sources[i];
                                ...
                                #ifndef DEF_this_cpp_lkLAWNEVGo2
                                #define DEF_this_cpp_lkLAWNEVGo2
inline static fu::view<char> this_cpp_lkLA(const ModuleOutputs_GBmK& out)
{
    return out.cpp.src;
}
                                #endif

static fu::str all_cpp_3QGL(const Context_Zsw9& ctx)
{
    return join_yqDb(map_4Hg5(ctx.modules), "\n----\n"_view);
}

                                #ifndef DEF_STOP_T4lZeXqSoT7
                                #define DEF_STOP_T4lZeXqSoT7
[[noreturn]] inline static fu::never STOP_T4lZ(fu::view<char> reason, fu::view<char> x, const Context_Zsw9& ctx, fu::view<fu::str> sources)
{
...
inline bool operator!=(fu::view<char> a, fu::view<char> b)
{
    return !!x3Cx3E_j5CR(a, b);
                                ...
                                #ifndef DEF___akzJonq6cEe
                                #define DEF___akzJonq6cEe
inline static void _akzJ(fu::view<char> x, const Context_Zsw9& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    ...
    else
    {
        ...
        else
        {
            ...
            for (int i = 0; i < sources.size(); i++)
            {
                const ModuleOutputs_GBmK& out = ctx.modules[(i + 1)].out;
                if (note)
                {
                ...
                else if ((found = has_YIf6(this_cpp_lkLA(out), rest)))
                    break;
            ...
            {
                if (!(found))
                    STOP_T4lZ("EXPECT mismatch"_view, x, ctx, sources);

            }
            else if (cmd == "BROKEN"_view)
            {
                if (found)
                    STOP_T4lZ("STILL BROKEN"_view, x, ctx, sources);
                else
                    STOP_T4lZ("BROKEN mismatch"_view, x, ctx, sources);

            }
            else if (cmd != "TODO"_view)
                STOP_T4lZ("Unknown ;; CHECK"_view, x, ctx, sources);
            else if (found)
                STOP_T4lZ("TODO test is actually passing"_view, x, ctx, sources);
                                ...
                                #ifndef DEF_each_Y8HpHtbnJD1
                                #define DEF_each_Y8HpHtbnJD1
inline void each_Y8Hp(fu::view<fu::str> a, const Context_Zsw9& ctx, int& expect_alloc_count, int& expect_alloc_bytes, fu::view<fu::str> sources)
{
    for (int i = 0; i < a.size(); i++)
        _akzJ(a[i], ctx, expect_alloc_count, expect_alloc_bytes, sources);
...
static TODO_FIX_Tuple_fZNP compile_build_and_run_3QGL(fu::view<fu::str> sources, const Options_TBgD& options, const int expect_exit, const DEV_OnFail_3YLB onfail, const CGDefects_2L18 ok_defects, fu::view<fu::str> expectations)
{
    /*MOV*/ Context_Zsw9 ctx = compile_snippets_3QGL(sources, fu::vec<fu::str>{}, Options_TBgD(options));
    int expect_alloc_count = 0;
    int expect_alloc_bytes = 0;
    each_Y8Hp(expectations, ctx, expect_alloc_count, expect_alloc_bytes, sources);
    ...
    return TODO_FIX_Tuple_fZNP { static_cast<Context_Zsw9&&>(ctx), TestOutput_OIXu(debug_output.output) };
}
...
Context_Zsw9 ZERO_3QGL(fu::vec<fu::str>&& sources, Options_TBgD&& options, fu::vec_range<char> expect_fail, const int expect_exit, TestDiffs_EVlo& testdiffs)
{
    for (int i = 0; i < sources.size(); i++)
    {
        for (; ; )
...
;
    ...
    {
        /*MOV*/ TODO_FIX_Tuple_fZNP _ = compile_build_and_run_3QGL(sources, options, expect_exit, onfail, ok_defects, expectations);

        {
            ...
            {
                input += sources[i_5];
                const Module_cdtC& m = _.ctx.modules[((i_5 + _.ctx.modules.size()) - sources.size())];
        ...
        return static_cast<Context_Zsw9&&>(_.ctx);
    };
}
...
{
    fu::vec<Module_cdtC> expect = ZERO_3QGL(fu::vec<fu::str>(alts[0]), Options_TBgD{}, (*(fu::str*)fu::NIL), 0, testdiffs).modules;
    for (int i = 1; i < alts.size(); i++)
    {
        fu::vec<Module_cdtC> actual = compile_snippets_3QGL(alts[i], fu::vec<fu::str>{}, Options_TBgD{}).modules;
...
Context_Zsw9 ZERO_5TaN(fu::vec_range<char> src, TestDiffs_EVlo& testdiffs)
{
