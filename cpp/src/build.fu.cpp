...
inline constexpr CacheCleanup_Oz5J MASK_CacheCleanup_Oz5J
    ...
    | CacheCleanup_Oz5J_Cpp;
                                ...
                                #ifndef DEF_TranslationUnit_ZY9D67BzZXc
                                #define DEF_TranslationUnit_ZY9D67BzZXc
struct TranslationUnit_ZY9D
{
    fu::str human;
    fu::str src;
    fu::str iquote;
    TEA_0Daz self_src_hash;
...
struct Map_cOsg
{
    ...
    explicit operator bool() const noexcept
    {
        ...
        ;
                                ...
                                #ifndef DEF_DirWrk_q6zYPxkpTwe
                                #define DEF_DirWrk_q6zYPxkpTwe
struct DirWrk_q6zY
{
    fu::str cpp;
    fu::str obj;
    fu::str bin;
    fu::str tmp;
    DirWrk_q6zY(const DirWrk_q6zY&) = delete;
    DirWrk_q6zY(DirWrk_q6zY&&) = default;
    DirWrk_q6zY& operator=(const DirWrk_q6zY&) = delete;
    DirWrk_q6zY& operator=(DirWrk_q6zY&&) = default;
    explicit operator bool() const noexcept
    {
        return false
            || cpp
...
static const fu::str TODO_FIX_ignoredWarnings fu_INIT_PRIORITY(1006) = ((((fu::view<char>{} + "-Wno-float-conversion "_view) + "-Wno-unused-but-set-variable "_view) + "-Wno-unknown-warning-option "_view) + "-Wno-maybe-uninitialized -Wno-stringop-overflow -Wno-array-bounds -Wno-sequence-point -Wno-dangling-reference "_view);
                                ...
                                #ifndef DEF___NlOxldVt3m8
                                #define DEF___NlOxldVt3m8
inline static int _NlOx()
{
    return -1;
}
                                #endif

                                #ifndef DEF_map_G8GmKLOjbz1
                                #define DEF_map_G8GmKLOjbz1
inline fu::vec<int> map_G8Gm(fu::view<Module_nyjb> a)
{
    /*MOV*/ fu::vec<int> res {};
...
inline bool add_JHtk(Set_XshD& _, fu::vec_range<char> key)
{
    ...
    return true;
                                ...
                                #ifndef DEF___QtCcGzeeA62
                                #define DEF___QtCcGzeeA62
inline static bool _QtCc(fu::vec_range<char> incl, TranslationUnit_ZY9D& unit, fu::view<char> dir)
{
    return add_JHtk(unit.hacks.include_dirs, (starts_Sfbf(incl, '.') ? join_qZI0(dir, incl) : fu::str(incl)));
}
                                #endif

                                #ifndef DEF_each_8uKb5OiA7qc
                                #define DEF_each_8uKb5OiA7qc
inline void each_8uKb(fu::view<fu::str> a, TranslationUnit_ZY9D& unit, fu::view<char> dir)
{
    for (int i = 0; i < a.size(); i++)
        _QtCc(a[i], unit, dir);
...
static DirWrk_q6zY DirWrk_uGHE(fu::view<char> dir_wrk)
{
    /*MOV*/ DirWrk_q6zY ret = DirWrk_q6zY { (dir_wrk + "cpp/"_view), (dir_wrk + "obj/"_view), (dir_wrk + "bin/"_view), (dir_wrk + "tmp/"_view) };
...
inline static void link_each_8txf(const Set_XshD& link, Set_XshD& HACK_linkFlags, Set_XshD& HACK_pkgConfig_libs)
{
    for (int i = 0; i < link.keys_asc.size(); i++)
    {
        ...
        _b2TS((isFlag ? item : (*(fu::str*)fu::NIL)), (!isFlag ? item : (*(fu::str*)fu::NIL)), HACK_linkFlags, HACK_pkgConfig_libs);
                                ...
                                #ifndef DEF___kz2OH7HmAc8
                                #define DEF___kz2OH7HmAc8
inline static void _kz2O(const TranslationUnit_ZY9D& unit, Set_XshD& HACK_linkFlags, Set_XshD& HACK_pkgConfig_libs, Set_XshD& HACK_include_dirs, int& len_all, TEA_0Daz& hash_all)
{
    ...
    link_each_8txf(unit.hacks.link, HACK_linkFlags, HACK_pkgConfig_libs);
                                ...
                                #ifndef DEF_each_mcuxmQ1nCvk
                                #define DEF_each_mcuxmQ1nCvk
inline void each_mcux(fu::view<TranslationUnit_ZY9D> a, Set_XshD& HACK_linkFlags, Set_XshD& HACK_pkgConfig_libs, Set_XshD& HACK_include_dirs, int& len_all, TEA_0Daz& hash_all)
{
    for (int i = 0; i < a.size(); i++)
        _kz2O(a[i], HACK_linkFlags, HACK_pkgConfig_libs, HACK_include_dirs, len_all, hash_all);
...
static fu::str tmpfile_uGHE(const DirWrk_q6zY& dir_wrk)
{
...
static Errno_88RA atomic_write_uGHE(fu::vec_range<char> fname, fu::view<char> data, const DirWrk_q6zY& dir_wrk)
{
...
static BuildError_kI1v buildUnit_uGHE(TranslationUnit_ZY9D& unit, const int i, const RunMode_n0r3 runmode, const BuildScheme_Vgo7 bscheme, fu::view<char> GCC_CMD, fu::view<char> INCLUDE, const DirWrk_q6zY& dir_wrk)
{
    ...
    else
    {
        ...
        else
        {
            ...
            return BuildError_kI1v{};
                                ...
                                #ifndef DEF___oARKBq58k39
                                #define DEF___oARKBq58k39
inline static const BuildError_kI1v& _oARK(const int i, fu::view_mut<TranslationUnit_ZY9D> arr, fu::view_mut<BuildError_kI1v> result, const RunMode_n0r3 runmode, const BuildScheme_Vgo7 bscheme, fu::view<char> GCC_CMD, fu::view<char> INCLUDE, const DirWrk_q6zY& dir_wrk)
{
    TranslationUnit_ZY9D* _0;
    return (result.mutref(i) = (_0 = &(arr.mutref(i)), buildUnit_uGHE(*_0, i, runmode, bscheme, GCC_CMD, INCLUDE, dir_wrk)));
}
                                #endif

                                #ifndef DEF_parallel_for_iaIRImtOOl4
                                #define DEF_parallel_for_iaIRImtOOl4
inline static void parallel_for_iaIR(const int min, int end, fu::view_mut<TranslationUnit_ZY9D> arr, fu::view_mut<BuildError_kI1v> result, const RunMode_n0r3 runmode, const BuildScheme_Vgo7 bscheme, fu::view<char> GCC_CMD, fu::view<char> INCLUDE, const DirWrk_q6zY& dir_wrk)
{
    int start = (end & 0);
    (void) start;
    ...
    for (int i = start; i < end; i++)
        _oARK(i, arr, result, runmode, bscheme, GCC_CMD, INCLUDE, dir_wrk);
                                ...
                                #ifndef DEF_map_TztUTOPQo15
                                #define DEF_map_TztUTOPQo15
inline fu::vec<BuildError_kI1v> map_TztU(fu::view_mut<TranslationUnit_ZY9D> arr, const int min, const RunMode_n0r3 runmode, const BuildScheme_Vgo7 bscheme, fu::view<char> GCC_CMD, fu::view<char> INCLUDE, const DirWrk_q6zY& dir_wrk)
{
    /*MOV*/ fu::vec<BuildError_kI1v> result {};
    result.grow<false>(arr.size());
    parallel_for_iaIR(min, arr.size(), arr, result, runmode, bscheme, GCC_CMD, INCLUDE, dir_wrk);
    return /*NRVO*/ result;
}
                                #endif

                                #ifndef DEF___q3ud1DtJYya
                                #define DEF___q3ud1DtJYya
inline static CacheCleanup_Oz5J _q3ud(TranslationUnit_ZY9D& unit)
{
    return (unit.cache_cleanup &= CacheCleanup_Oz5J(~CacheCleanup_Oz5J_Cpp));
}
                                #endif

                                #ifndef DEF_each_f3Gbical1Bi
                                #define DEF_each_f3Gbical1Bi
inline void each_f3Gb(fu::view_mut<TranslationUnit_ZY9D> a)
{
    for (int i = 0; i < a.size(); i++)
        _q3ud(a.mutref(i));
                                ...
                                #ifndef DEF___COq7u2HIjYd
                                #define DEF___COq7u2HIjYd
inline static fu::view<char> _COq7(const TranslationUnit_ZY9D& unit, fu::str& explain)
{
    return (explain += (unit.human + "\n"_view));
}
                                #endif

                                #ifndef DEF_each_iQdKkcTRqT8
                                #define DEF_each_iQdKkcTRqT8
inline void each_iQdK(fu::view<TranslationUnit_ZY9D> a, fu::str& explain)
{
    for (int i = 0; i < a.size(); i++)
        _COq7(a[i], explain);
                                ...
                                #ifndef DEF___lAJLSYdYcil
                                #define DEF___lAJLSYdYcil
inline static fu::view<char> _lAJL(const TranslationUnit_ZY9D& unit, fu::str& explain)
{
    return (explain += unit.src);
}
                                #endif

                                #ifndef DEF_each_0BLg8LRA1oj
                                #define DEF_each_0BLg8LRA1oj
inline void each_0BLg(fu::view<TranslationUnit_ZY9D> a, fu::str& explain)
{
    for (int i = 0; i < a.size(); i++)
        _lAJL(a[i], explain);
...
[[noreturn]] static fu::never ERR_uGHE(fu::str&& cpp, fu::view_mut<TranslationUnit_ZY9D> units, const DEV_OnFail_3YLB onfail, const int code, fu::str& stdout, const DirWrk_q6zY& dir_wrk)
{
    each_f3Gb(units);
    if (onfail & DEV_OnFail_3YLB_OnFail_WriteRepro)
    {
        if (!cpp)
        {
            ...
            {
                const TranslationUnit_ZY9D& unit = units[i];
    ...
    if (onfail & DEV_OnFail_3YLB_OnFail_PrintInput)
    {
        ...
        each_iQdK(units, explain);
    };
    if (onfail & DEV_OnFail_3YLB_OnFail_PrintOutput)
    {
        explain += "\n\n\tGenerated code:\n\n"_view;
        each_0BLg(units, explain);
    };
...
static void WRITE_cachedTestOutput_uGHE(fu::vec_range<char> F_exe, const TestOutput_OIXu& output, const DirWrk_q6zY& dir_wrk)
{
...
static BuildOutput_zaam buildTarget_uGHE(fu::view_mut<TranslationUnit_ZY9D> units, fu::view<FileInvariants_fbOC> file_invariants, fu::view<fu::str> flags_cc, fu::view<fu::str> flags_ld, const DEV_OnFail_3YLB onfail, const RunMode_n0r3 runmode, const int expect_exit, const BuildScheme_Vgo7 bscheme, fu::vec_range<char> bin, fu::view<char> GCC_CMD, const Context_GwtN& ctx, const DirWrk_q6zY& dir_wrk)
{
    ...
    int len_all {};
    TEA_0Daz hash_all {};
    each_mcux(units, HACK_linkFlags, HACK_pkgConfig_libs, HACK_include_dirs, len_all, hash_all);
    ...
    const int64_t old_size = (!BuildScheme_Vgo7((bscheme & BuildScheme_Vgo7_nocache)) ? size_VWJw(fu::str(F_exe)) : 0ll);
    const int64_t old_size_1 = ((old_size != 4ll) ? old_size : 0ll);
    if ((old_size_1 < 1ll) && (bin || runmode))
    {
        fu::vec<BuildError_kI1v> buildErrors = map_TztU(units, 0, runmode, bscheme, GCC_CMD, INCLUDE, dir_wrk);
        for (int i_1 = 0; i_1 < buildErrors.size(); i_1++)
        {
            ...
            {
                stdout = err.stdout;
                ERR_uGHE(fu::str(err.cpp), units, onfail, code, stdout, dir_wrk);
            };
        };
        int _didCrosscheckHeaders {};
        for (int i_2 = 0; i_2 < units.size(); i_2++)
        {
            TranslationUnit_ZY9D& unit = units.mutref(i_2);
            if (unit.tmp_file)
            {
        ...
        if (bscheme & BuildScheme_Vgo7_shared)
        {
            cmd += " -shared"_view;
            fu::view<char> soname = filename_qZI0(bin);
            if (fu::APPLE)
                cmd += (" -Wl,-install_name,@rpath/"_view + soname);
            else
        ...
        {
            const TranslationUnit_ZY9D& unit = units[i_3];
...

        {
            ...
            if (code)
            {
                ...
                ERR_uGHE(fu::str{}, units, onfail, code, stdout, dir_wrk);
            }
            else
            {
                const double t1 = hr_0pZh();
    ...
    {
        const bool OPTI_StatusCode = (runmode == RunMode_n0r3_CompilerTestcase);
        ExitStatus_iZrm status {};
        ...
        else
        {

            {
                ...
                fu::str stdout_1 {};
                const double t0 = hr_0pZh();
                code = int(spawn_0Hw8(fu::vec<fu::str> {{ fu::str(F_exe) }}, stdout_1, stdout_1, status, (runmode == RunMode_n0r3_Normally)));
            ...
            if (OPTI_StatusCode)
            {
                ...
                if (looks_legit_6GsP(new_size))
                    ...
                    WRITE_cachedTestOutput_uGHE(F_exe, output, dir_wrk);
...
static fu::vec_range<char> update_file_IpIP(fu::vec_range<char> fname, fu::view<char> data, fu::view<char> dir_cpp, const DirWrk_q6zY& dir_wrk)
{
    fu::str xcheck {};
...
static fu::str update_file_uGHE(fu::vec_range<char> fname, fu::view<char> data, fu::view<char> dir_src, fu::view<char> dir_cpp, const DirWrk_q6zY& dir_wrk)
{
    return update_file_IpIP(update_file_dest_uGHE(fname, dir_src, dir_cpp), data, dir_cpp, dir_wrk).const_cast_mut().destructive_move();
}

static fu::view<char> update_file_QLsA(fu::vec_range<char> fname, fu::view<char> data, fu::view<char> dir_cpp, const DirWrk_q6zY& dir_wrk)
{
    fu::str xcheck {};
    if (read_VWJw(fu::str(fname), xcheck, 0ll) || (xcheck != data))
    {
...
static void outputSources_uGHE(fu::view<TranslationUnit_ZY9D> units, fu::view<char> dir_src, fu::vec_range<char> dir_cpp, fu::view<char> unity, const DirWrk_q6zY& dir_wrk)
{
    if (dir_cpp && dir_src)
    {
...
BuildOutput_zaam build_uGHE(fu::str&& dir_wrk, fu::str&& fulib, fu::vec_range<char> bin, fu::str&& dir_obj, fu::str&& dir_src, fu::str&& dir_cpp, fu::view<char> unity, const DEV_OnFail_3YLB onfail, const RunMode_n0r3 runmode, const int expect_exit, const BuildScheme_Vgo7 bscheme, fu::view<fu::str> flags_cc, fu::view<fu::str> flags_ld, const Context_GwtN& ctx)
{
    ...
    if (!BuildScheme_Vgo7((bscheme & BuildScheme_Vgo7_unoptimized)))
    ...
    {
        CodegenOutput_hNw4 fulib_cpp_1 = CodegenOutput_hNw4 { static_cast<fu::str&&>(fulib_cpp), Set_XshD{}, BuildHacks_sqc0{}, CGDefects_2L18{} };
        fu::vec<int> unit_indices = map_G8Gm(ctx.modules);
        fu::vec<TranslationUnit_ZY9D> units {};
        ...
        for (int i = 0; i < dep_order.size(); i++)
        {
            ...
            const CodegenOutput_hNw4& cpp = (i ? module.out.cpp : fulib_cpp_1);
            if (!(!cpp.src))
            {
                ...
                units.push(TranslationUnit_ZY9D { fu::str(human), fu::str(cpp.src), fu::str(iquote), TEA_0Daz{}, TEA_0Daz{}, (fname + ".cpp"_view), fu::str{}, TEA_0Daz{}, fu::str{}, fu::str{}, CacheCleanup_Oz5J{}, BuildHacks_sqc0{} });
                const int unit_index = (unit_indices.mutref(modid) = (units.size() - 1));
                TranslationUnit_ZY9D& unit = units.mutref(unit_index);
                ...
                if (cpp.hacks)
                {
                    ...
                    if (include_dirs)
                    {
                        ...
                        each_8uKb(include_dirs.keys_asc, unit, dir);
                    };
                };
            };
        };
        DirWrk_q6zY dir_wrk_1 = DirWrk_uGHE(dir_wrk);
        for (int i_1 = 0; i_1 < units.size(); i_1++)
        {
            TranslationUnit_ZY9D& unit = units.mutref(i_1);
            fu::view<char> cpp = (unit.src ? unit.src : fail_uGHE(str_fhzi64hBLPa));
            fu_ASSERT(!unit.self_src_hash);
            hash_l6RU(unit.self_src_hash, unit.src);
            fu_ASSERT(!unit.obj_hash);
            TEA_0Daz& hash = (unit.obj_hash = unit.self_src_hash);
            const TEA_0Daz* tea;
        ...
        /*MOV*/ BuildOutput_zaam ret = buildTarget_uGHE(units, file_invariants, flags_cc, flags_ld, onfail, runmode, expect_exit, bscheme, bin, GCC_CMD, ctx, dir_wrk_1);
