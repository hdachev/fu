...
struct Type_OiTm
{
    ...
    explicit operator bool() const noexcept
    {
        ...
        ;
                                ...
                                #ifndef STR_pDroh3OoVHc
                                #define STR_pDroh3OoVHc
static const fu::str str_pDroh3OoVHc fu_INIT_PRIORITY(1007) { "createUserTypeCanon: bad modid/index"_fu };
                                #endif
...
UserTypeCanon_rNcg parseUserTypeCanon_27MV(fu::view<char> canon)
{
    int offset = 1;
    ...
    {
        return UserTypeCanon_rNcg { modid, (globid - ONE_BASED) };
    }
    else
        fu::fail(x7E_rA00((x7E_rA00((("parseUserTypeCanon: trailing garbage in `"_view + canon) + "`: "_view), fu::i64dec(offset)) + "|"_view), fu::i64dec(canon.size())));

}

fu::str createUserTypeCanon_27MV(const Kind_Idfg kind, fu::view<char> basePrim, const int modid, const int index, fu::view<char> specPat, uint64_t hash)
{
    hash ^= (hash >> 32ull);
    hash ^= (hash >> 16ull);
    hash ^= (hash >> 8ull);
    const char sigil = char((hash | 128ull));
    /*MOV*/ fu::str res = (basePrim + char(sigil));
    if ((modid >= 0) && (index >= 0))
    {
        ...
        if (basePrim)
        {
            char& c = res.mutref(0);
            if ((c == 'e') || (c == 'm'))
                fu::fail(("createUserTypeCanon: bad basePrim: "_view + basePrim));
        ...
        fu::fail(fu::str(str_pDroh3OoVHc));

}

bool isUserType_27MV(const ValueType_JtNg& type)
{
    const char c = type.canon[0];
    return (fu::u8(fu::u8(c)) >= fu::u8(128u));
}

int modidOfOrigin_27MV(const Type_OiTm& type)
{
    if (isUserType_27MV(type.vtype))
    {
