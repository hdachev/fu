...
static void inspect_zVz2(const SolvedNode_efhg&, fu::view<char>, const Module_BOS4&, fu::str&, int);
...
static void inspect_zVz2(const SolvedNode_efhg& n, fu::view<char> indent, const Module_BOS4& module, fu::str& src, const int modid)
{
    src += (indent + "("_view);
    if (n)
    {
        ...
        if (((t_2 = &(n.target), int(unsigned((((*t_2)._packed >> 40ull) & 0xfffffull)))) == modid) || locid)
        {
            ...
            {
                const Extended_wKyO& x = module.out.solve.scope.extended[globid];
                if (x.fx_mask)
...
fu::str inspect_WenQ(const Module_BOS4& module, const SolvedNode_efhg& root)
{
...
fu::str inspect_wkVO(const Module_BOS4& module)
{
